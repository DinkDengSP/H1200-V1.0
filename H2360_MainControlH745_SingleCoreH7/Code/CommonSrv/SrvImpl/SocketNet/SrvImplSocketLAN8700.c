...
** LastEditTime  : 2023-11-10 10:51:09 +0800
************************************************************************************************/ 
...
static uint32_t SrvImplSocketLAN8700GetIP(void)
{
    ...
    {
        //读取失败,用默认IP
        SystemPrintf("BoardParamReadNetPhyIP_Addr Failed,ErrorSub: 0X%08X\r\n",errorSub);
        phyNetIp = DEFAULT_VALUE_BOARD_PARAM_NET_PHY1_IP;
    }
    SystemPrintf("LAN8700 IP: 0X%08X\r\n",phyNetIp);
    return phyNetIp;
}
...
//创建探针网口
void SrvImplSocketLAN8700CreateSocketProbe(void)
{
    uint32_t nxResult;
    //创建TCP Socket
    ...
    {
        nxResult = nx_tcp_socket_create(&lan8700NetX,              //IP实例控制块 */    
                                        SocketNetStateGetProbeSocketPtr(),  //TCP控制块 */ 
                                        "tcpSocketProbe",             //TCP Socket名 */ 
                                        NX_IP_NORMAL|NX_IP_MIN_DELAY|NX_IP_MAX_RELIABLE,//IP服务类型 
                                        NX_FRAGMENT_OKAY,           //使能IP分段 */ 
                                        NX_IP_TIME_TO_LIVE,         //指定一个 8 位的值，用于定义此数据包在被丢弃之前可通过的路由器数目
                                        TCP_SERVICE_PROBE_SOCKET_BUF_LENGTH, //TCP Socket接收队列中允许的最大字节数
                                        NX_NULL,                    //用于在接收流中检测到紧急数据时调用的回调函数
                                        NX_NULL);                   //TCP Socket另一端发出断开连接时调用的回调函数
        if(nxResult != NX_SUCCESS)
        {
...
void SrvImplSocketLAN8700StackInit(void)
{
    ...
    //创建探针通讯Socket
    SrvImplSocketLAN8700CreateSocketProbe();
    //打印初始化完成日志
    SystemPrintf("%s Success Over\r\n",__func__);
    //设置初始化完成
...
//重建探针端口
static void SrvImplSocketLAN8700ProbePortReCreate(void)
{
    uint32_t nxResult;
    ...
    do
    {
        nxResult = nx_tcp_socket_disconnect(SocketNetStateGetProbeSocketPtr(),NX_WAIT_FOREVER);
        if(NX_NOT_CONNECTED == nxResult)
        {
    ...
    nxResult = nx_tcp_server_socket_unaccept(SocketNetStateGetProbeSocketPtr());
    if(nxResult != NX_SUCCESS)
    {
    ...
    nxResult = nx_tcp_server_socket_unlisten(&lan8700NetX, TCP_PROBE_SOCKET_PORT);
    if(nxResult != NX_SUCCESS)
    {
    ...
    nxResult = nx_tcp_socket_delete(SocketNetStateGetProbeSocketPtr());
    if(nxResult != NX_SUCCESS)
    {
    ...
    SrvImplSocketLAN8700CreateSocketProbe();
    //读取Socket状态
    nxResult = SrvImplSocketLAN8700PortSocketInfoGet(SocketNetStateGetProbeSocketPtr(),SocketNetStateGetProbeSocketInfoPtr());
    if(nxResult != NX_SUCCESS)
    {
    ...
    //状态复制
    SocketNetStateSetProbeSocketStateCurrentByRecord(SocketNetStateGetProbeSocketStateByInfo());
    SocketNetStateSetProbeSocketStateLastByRecord(SocketNetStateGetProbeSocketStateByInfo());
}
...
void SrvImplSocketLAN8700PhyLinkFSM(void)
{
    ...
    if((lan8700NetBasePtr->lanPhyConnectStateCurrent == NX_FALSE)&&(lan8700NetBasePtr->lanPhyConnectStateLast == NX_TRUE))
    {
        ...
        //更新探针Socket状态
        SrvImplSocketLAN8700PortSocketInfoGet(SocketNetStateGetProbeSocketPtr(),SocketNetStateGetProbeSocketInfoPtr());
        //如果主网口处于链接状态,关闭连接
        ...
        //如果探针网口处于链接状态,关闭链接
        if(SocketNetStateGetProbeSocketStateByInfo() == NX_TCP_ESTABLISHED)
        {
            SrvImplSocketLAN8700ProbePortReCreate();
        }
    }
...
void SrvImplSocketLAN8700MainPortFSM(void)
{
    ...
    if(SocketNetStateGetMainSocketStateCurrentByRecord() != SocketNetStateGetMainSocketStateLastByRecord())
    {
        ...
        //状态变化时候,修改系统联机状态
        SYS_STATE_HEART* sysCommHeartStatePtr = SysStateGetCommHeart();
        if(SocketNetStateGetMainSocketStateCurrentByRecord() != NX_TCP_ESTABLISHED)
        {
            //SOCKET断开,自动脱机
...
/*----------------------------------------探针网口通讯状态机------------------------------------------------*/
//Socket关闭状态下执行的状态机功能
static void SrvImplSocketLAN8700ProbePortFSM_Closed(void)
{
    uint32_t nxResult;
    //Socket已经关闭,切换Listen
    SystemPrintf("LAN8700 ProbePortFSM_Closed nx_tcp_server_socket_listen Start\r\n");
    nxResult = nx_tcp_server_socket_listen(&lan8700NetX,                //IP实例控制块 */  
                                            TCP_PROBE_SOCKET_PORT,        //监听端口 */          
                                            SocketNetStateGetProbeSocketPtr(),  //TCP Socket控制块
                                            TCP_PROBE_SOCKET_PORT_MAX_CLIENT,   //可以监听的连接数
                                            NULL);                         //监听回调函数
    if(nxResult != NX_SUCCESS)
    {
        //监听失败
...
//监听状态下,启用新的链接
static void SrvImplSocketLAN8700ProbePortFSM_Listen(uint32_t lastSocketState)
{
    uint32_t nxResult;
    //持续监听
    if((lastSocketState == NX_TCP_SYN_RECEIVED)||(lastSocketState == NX_TCP_LISTEN_STATE))
    {
        ...
        nxResult = nx_tcp_server_socket_relisten(&lan8700NetX, TCP_PROBE_SOCKET_PORT, SocketNetStateGetProbeSocketPtr());
        if(nxResult != NX_SUCCESS)
        {
    ...
    nxResult = nx_tcp_server_socket_accept(SocketNetStateGetProbeSocketPtr(),NX_NO_WAIT);
    if(nxResult == NX_IN_PROGRESS)
    {
        //正在处理,目前不用管
...
//链接关闭的情况下执行的操作
static void SrvImplSocketLAN8700ProbePortFSM_CloseWait(void)
{
    uint32_t nxResult;
    //解除Socket和服务器端口的绑定
...
//链接已经建立的情况下执行的操作
static void SrvImplSocketLAN8700ProbePortFSM_Established(void)
{
    uint32_t nxResult;
    //读取到了数据,解析出来的接受数据的长度
    ULONG recvDataLength = 0;
    ...
    while(1)
    {
        ...
        netRecvPackPtr = NULL;
        //到这里,连接已经建立,接收数据包
        nxResult = nx_tcp_socket_receive(SocketNetStateGetProbeSocketPtr(),&tcpSocketProbePortRecvDataPackPtr, NX_NO_WAIT);  
        if(nxResult != NX_SUCCESS)
        {
            //没有数据或者数据读取失败
            break;
        ...
        UserMemClear((void*)recvDataBufferPtr,recvDataLength);
        //获取远程数据IP和远程数据端口号
        nx_tcp_socket_peer_info_get(SocketNetStateGetProbeSocketPtr(),&peerIpAddressProbePort,&peerPortProbePort); 
        //获取客户端发来的数据
...
//日志通讯端口状态机
void SrvImplSocketLAN8700ProbePortFSM(void)
{
    uint32_t nxResult;
    //读取Socket状态
    nxResult = SrvImplSocketLAN8700PortSocketInfoGet(SocketNetStateGetProbeSocketPtr(),SocketNetStateGetProbeSocketInfoPtr());
    ...
    SocketNetStateSetProbeSocketStateCurrentByRecord(SocketNetStateGetProbeSocketStateByInfo());
    //打印状态标记
    if(SocketNetStateGetProbeSocketStateCurrentByRecord() != SocketNetStateGetProbeSocketStateLastByRecord())
    {
        //打印状态变化
        SystemPrintf("LAN8700 ProbePortStateChange,Current: %s,Last: %s\r\n",
                            SocketNetStateGetSocketState(SocketNetStateGetProbeSocketStateCurrentByRecord()),
                            SocketNetStateGetSocketState(SocketNetStateGetProbeSocketStateLastByRecord()));
    }
    //打印相关Socket信息
    switch(SocketNetStateGetProbeSocketStateCurrentByRecord())
    {
        case NX_TCP_CLOSED:
            //Closed状态下执行的操作
            SrvImplSocketLAN8700ProbePortFSM_Closed();
            break;
        case NX_TCP_LISTEN_STATE:
            //Listen状态下执行的操作
            SrvImplSocketLAN8700ProbePortFSM_Listen(SocketNetStateGetProbeSocketStateLastByRecord());
            break;
        case NX_TCP_SYN_SENT:
            //同步发送状态
            break;
        ...
        case NX_TCP_ESTABLISHED:
            //已连接状态,数据读取
            SrvImplSocketLAN8700ProbePortFSM_Established();
            break;
        case NX_TCP_CLOSE_WAIT:
            //链接断开的情况下执行的操作
            SrvImplSocketLAN8700ProbePortFSM_CloseWait();
            break;
        case NX_TCP_FIN_WAIT_1:
            break;
        case NX_TCP_FIN_WAIT_2:
    ...
    //设置上一次状态和当前状态相同
    SocketNetStateSetProbeSocketStateLastByRecord(SocketNetStateGetProbeSocketStateCurrentByRecord());
    return;
}
...
//探针网口数据发送
uint32_t SrvImplSocketLAN8700ProbeSendBuf(uint8_t* bufferPtr,uint32_t bufLength)
{
    //首先查看网口状态
    uint32_t nxResult;
    //获取互斥信号量
    NetStateProbePortDataSendGetLock();
    //读取Socket状态
    nxResult = SrvImplSocketLAN8700PortSocketInfoGet(SocketNetStateGetProbeSocketPtr(),SocketNetStateGetProbeSocketInfoPtr());
    if(nxResult != NX_SUCCESS)
    {
        //释放互斥信号量
        NetStateProbePortDataSendReleaseLock();
        //日志打印
    ...
    //发出数据包
    nxResult =  nx_tcp_socket_send(SocketNetStateGetProbeSocketPtr(),sendPackDataPtr,TRANS_TIME_OUT_MS); 
    if(nxResult != NX_SUCCESS)
    {
        //释放互斥信号量
        NetStateProbePortDataSendReleaseLock();
...
//检测SOCKET MAIN连接情况
FlagStatus SrvImplSocketCheckConnectSta(void)
{
    ULONG socketState = SrvImplSocketLAN8700MainGetState();
...
//获取探针通讯网口状态
ULONG SrvImplSocketLAN8700ProbeGetState(void)
{
    //首先查看网口状态
    uint32_t nxResult;
    //获取网口互斥信号量
    ...
    nxResult = SocketNetStateGetProbeSocketStateByInfo();
    //释放网口互斥信号量
    NetStateProbePortDataSendReleaseLock();
    return nxResult;
}
...
void SrvImplNetSocketLAN8700CreateFtpServer(void)
{
    ...
    nxResult =  nx_ftp_server_create(&LAN8700FtpServer,//服务器对象
                                    ...
                                    SocketNetStateGetMemPoolPtr(), //服务器内存池
