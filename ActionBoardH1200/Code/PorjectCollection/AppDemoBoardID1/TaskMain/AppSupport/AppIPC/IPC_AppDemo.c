...
************************************************************************************************/ 
#include "IPC_AppDemo.h"
#include "AppTaskHeader.h"
#include "IPC_Port.h"

//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_APP_DEMO             128
//IPC通道讯息
static IPC_CHANNEL_DATA ipcChannelDataAppDemo;
//通道名称
static const uint8_t* ipcChannelAppDemoNameStr = "AppDemo";

/*****************************************************通道控制************************************************************/
//初始化IPC通道
void IPC_AppDemoInit(void)
{
    IPC_BaseInitChannelData(&ipcChannelDataAppDemo,(uint8_t*)ipcChannelAppDemoNameStr,
                                &queueAppTaskDemo,LENGTH_BUFFER_IPC_APP_DEMO);
}

//清空IPC信道
ERROR_SUB IPC_AppDemoClearChannelByCmd(uint32_t cmdCode)
{
    return IPC_BaseChannelClearChannel(&ipcChannelDataAppDemo,cmdCode);
}
...
ERROR_SUB IPC_AppDemoClearChannel(void)
{
    ...
    //首先清除通讯测试指令
    errorSub = IPC_BaseChannelClearChannel(&ipcChannelDataAppDemo,IPC_CMD_MACRO_COMM_TEST);
    ...
    {
        errorSub = IPC_BaseChannelClearChannel(&ipcChannelDataAppDemo,cmdCode);
        if(errorSub != ERROR_SUB_OK)
        {
...
void IPC_AppDemoWaitCmdReturn(uint32_t cmdCode,SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    //等待指令返回
    errorSub = IPC_BaseWaitCmdReturn(&ipcChannelDataAppDemo,cmdCode,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,Cmd: 0X%08X,ErrorSub: 0X%08X\r\n",__func__,cmdCode,errorSub);
        //IPC报错,返回错误
    ...
    if((ipcChannelDataAppDemo.channelCache.returnDataBufferLength != 0)&&(returnDataPtr != NULL))
    {
        //申请内存
        returnDataPtr->returnDataLength = ipcChannelDataAppDemo.channelCache.returnDataBufferLength;
        //不为0拷贝数据
        if(returnDataPtr->returnDataLength != 0)
        {
            ...
            //数据拷贝
            UserMemCopy(returnDataPtr->returnBufferPtr,ipcChannelDataAppDemo.channelCache.dataBufferPtr,returnDataPtr->returnDataLength);
        }
    }
    return;
}
...
void IPC_AppDemoWaitLastCmdReturn(SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    //等待指令返回
    errorSub = IPC_BaseWaitLastCmdReturn(&ipcChannelDataAppDemo,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
                                        ...
                                        IPC_RETURN_DATA* returnDataPtr)
{
    ...
    errorSub = IPC_BaseQueryCmdState(&ipcChannelDataAppDemo,cmdCode,cmdStagePtr,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,Cmd: 0X%08X,ErrorSub: 0X%08X\r\n",__func__,cmdCode,errorSub);
        //IPC报错,返回错误
...
void IPC_AppDemoQueryLastCmdState(IPC_CMD_STAGE* cmdStagePtr,SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    errorSub = IPC_BaseQueryLastCmdState(&ipcChannelDataAppDemo,cmdStagePtr,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,ErrorSub: 0X%08X\r\n",__func__,errorSub);
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_IPC_PROT_QUERY_CMD_STATE,ERROR_LEVEL_ERROR,
...
/*****************************************************发送指令等待ACK************************************************************/
static ERROR_SUB IPC_AppDemoCheckClearLastCmd(uint32_t cmdCode)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    PROT_CH_STA channelState;
    //首先检查是否上个数据未完成或者完成之后未读取
    channelState = IPC_BaseCheckCmdChannelState(&ipcChannelDataAppDemo,cmdCode);
...
void IPC_AppDemoCommTestWhileAck(SYS_RESULT_PACK* resultPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //清除上一个指令
    errorSub = IPC_AppDemoCheckClearLastCmd(IPC_CMD_MACRO_COMM_TEST);
    if(errorSub != ERROR_SUB_OK)
    {
    ...
    errorSub = IPC_BaseSendCmdWhileAck(&ipcChannelDataAppDemo,&ipcCmdBaseData);
    if(errorSub != ERROR_SUB_OK)
    {
...
void IPC_AppDemoCommTestWhileReturn(SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    ipcCmdBaseData.commandCode = IPC_CMD_MACRO_COMM_TEST;
    //清除掉上一次的指令了,进行下一次指令发送
    errorSub = IPC_BaseSendCmdWhileReturn(&ipcChannelDataAppDemo,&ipcCmdBaseData,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,Cmd: 0X%08X,ErrorSub: 0X%08X\r\n",__func__,ipcCmdBaseData.commandCode,
                        errorSub);
