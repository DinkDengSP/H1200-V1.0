...
void SrvMsgCan2FlashIapCmd(void* can2MsgBodyPtr)
{
    ...
    for(indexFlashIap = 0; indexFlashIap < DIM_ARRAY_ELEMENT_COUNT(can2CmdProcFlashIapUnitArray);indexFlashIap++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can2CmdProcFlashIapUnitArray[indexFlashIap].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexFlashIap == DIM_ARRAY_ELEMENT_COUNT(can2CmdProcFlashIapUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can2CmdProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can2CmdProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can2CmdProcFlashIapUnitArray[indexFlashIap].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can2CmdProcFlashIapUnitArray[indexFlashIap].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can2CmdProcFlashIapUnitArray[indexFlashIap].needResultReply == ENABLE)
    {
        //需要结果回传
...
//CAN上传消息处理函数列表
static const USER_CAN_UPLOAD_PROC_UNIT can2MsgUploadProcFlashIapUnitArray[] = 
{
...
void SrvMsgCan2FlashIapUpload(void* can2MsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexFlashIap = 0;
    for(indexFlashIap = 0; indexFlashIap < DIM_ARRAY_ELEMENT_COUNT(can2MsgUploadProcFlashIapUnitArray);indexFlashIap++)
    {
        if(canUploadPackPtr->uploadID == (uint32_t)(can2MsgUploadProcFlashIapUnitArray[indexFlashIap].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexFlashIap == DIM_ARRAY_ELEMENT_COUNT(can2MsgUploadProcFlashIapUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(canUploadPackPtr->uploadDataBufferLength < can2MsgUploadProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        can2MsgUploadProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin,canUploadPackPtr->uploadDataBufferLength,
    ...
    //检测函数指针
    if(can2MsgUploadProcFlashIapUnitArray[indexFlashIap].canUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    can2MsgUploadProcFlashIapUnitArray[indexFlashIap].canUploadProcFuncPtr(canUploadPackPtr);
    //处理完成,释放内存
    if((canUploadPackPtr->uploadDataBufferPtr != NULL)&&(canUploadPackPtr->uploadDataBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_CAN,canUploadPackPtr->uploadDataBufferPtr);
