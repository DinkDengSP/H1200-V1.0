...
** LastEditTime  : 2023-08-28 20:12:23 +0800
************************************************************************************************/ 
...
//任务的控制块
TX_THREAD tcbSrvTaskStartHeart@".ccram";
//任务堆栈
uint64_t stackBufferSrvTaskStartHeart[STK_SIZE_SRV_TASK_START_HEART/8]@".ccram";
//任务消息队列控制块
TX_QUEUE queueSrvTaskStartHeart@".ccram";
//消息缓冲区
uint32_t queueBufferSrvTaskStartHeart[LENGTH_QUEUE_SRV_TASK_START_HEART]@".ccram";
...
void SrvTaskStartHeartFuncEntry(ULONG threadInput)
{
    ...
    //公共服务层数据初始化
    CommonSrvImplStartHeartCreateSrvData();
    //公共服务层任务初始化
    CommonSrvImplStartHeartCreateSrvTask();
    //服务层数据初始化
    ...
    CoreDelayMs(20);
    //系统启动完成,打印编译信息
    SystemPrintf("STM32F407IGT6 ThreadX ActSlaveJQ1 App Start BoardID: %d\r\n",BoardIdentityPortGet());
    //打印编译信息
