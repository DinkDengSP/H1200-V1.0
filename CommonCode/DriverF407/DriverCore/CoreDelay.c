...
** LastEditTime  : 2023-12-05 10:12:44 +0800
************************************************************************************************/ 
#include "CoreDelay.h"
#include "tx_api.h"
...
//DWT初始化
static void CoreDelayInitDWT(void)
{
    //解锁DWT的访问
...
#if(CORE_DELAY_MODE_USER == CORE_DELAY_MODE_TIMER6)
...
#error "USE TIM6 Set CodeDelay,MCU_TIMER6_MODE_USER Need Set Mode MCU_TIMER6_MODE_BASE"
#endif
//单次延时最大微秒数
...
#if(CORE_DELAY_MODE_USER == CORE_DELAY_MODE_TIMER7)
#include "MCU_Timer7.h"
extern  volatile ULONG  _tx_thread_system_state;

//检查定时器设置
#if(MCU_TIMER7_MODE_USER != MCU_TIMER7_MODE_BASE)
#error "USE TIM7 Set CodeDelay,MCU_TIMER7_MODE_USER Need Set Mode MCU_TIMER7_MODE_BASE"
#endif
//单次延时最大微秒数
#define TIMER_DELAY_MAX_US_SINGLE       60000
static uint32_t overFlowFlag = 0;
...
{
    int32_t startTimeTick = tx_time_get();
    do
    {
        //循环延时
        CoreDelayMs(5);
    }while(OS_TICK_TO_MS(tx_time_get() - startTimeTick) < nms);
    return;
}
...
uint32_t HAL_GetTick(void)
{
    ...
    if (_tx_thread_system_state == TX_INITIALIZE_IS_FINISHED)
    {
        ticks = (uint32_t)_tx_time_get();
        ticks *= THREAD_X_TIME_CYCLE_MIN_MS;
        return ticks;
    }
