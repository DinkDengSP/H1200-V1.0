...
** LastEditTime  : 2023-11-29 14:23:32 +0800
************************************************************************************************/ 
...
//CAN指令消息处理函数
void SrvMsgCan1ParamCmd(void* can1MsgBodyPtr)
{
    //转换指令码
    ...
    for(indexParam = 0; indexParam < DIM_ARRAY_ELEMENT_COUNT(can1CmdProcParamUnitArray);indexParam++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can1CmdProcParamUnitArray[indexParam].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexParam == DIM_ARRAY_ELEMENT_COUNT(can1CmdProcParamUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can1CmdProcParamUnitArray[indexParam].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can1CmdProcParamUnitArray[indexParam].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can1CmdProcParamUnitArray[indexParam].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can1CmdProcParamUnitArray[indexParam].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can1CmdProcParamUnitArray[indexParam].needResultReply == ENABLE)
    {
        //需要结果回传
...
/*************************************************************CAN上传参数处理***********************************************************/
//占位函数
static void SrvMsgCan1ParamUpload_PlaceHolder(CAN_PROT_UPLOAD_PACK* canUploadPackPtr);

//CAN上传消息处理函数列表
static const USER_CAN_UPLOAD_PROC_UNIT can1MsgUploadProcParamUnitArray[] = 
{
...
//CAN上传消息处理函数
void SrvMsgCan1ParamUpload(void* can1MsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexParam = 0;
    for(indexParam = 0; indexParam < DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcParamUnitArray);indexParam++)
    {
        if(canUploadPackPtr->uploadID == (uint32_t)(can1MsgUploadProcParamUnitArray[indexParam].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexParam == DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcParamUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(canUploadPackPtr->uploadDataBufferLength < can1MsgUploadProcParamUnitArray[indexParam].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        can1MsgUploadProcParamUnitArray[indexParam].exceptParamLengthMin,canUploadPackPtr->uploadDataBufferLength,
    ...
    //检测函数指针
    if(can1MsgUploadProcParamUnitArray[indexParam].canUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    can1MsgUploadProcParamUnitArray[indexParam].canUploadProcFuncPtr(canUploadPackPtr);
    //处理完成,释放内存
    if((canUploadPackPtr->uploadDataBufferPtr != NULL)&&(canUploadPackPtr->uploadDataBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_CAN,canUploadPackPtr->uploadDataBufferPtr);
