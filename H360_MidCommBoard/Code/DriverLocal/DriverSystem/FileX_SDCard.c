...
** LastEditTime  : 2023-08-21 19:20:30 +0800
************************************************************************************************/ 
...
void FileSystemViewRootDir(uint8_t* mediaNameStr,FX_MEDIA* fxMidiaPtr)
{
    ...
    MCU_LogPrintf("======================%s FileSystem RootDirShow=======================\r\n",mediaNameStr);
    //打印table头
    MCU_LogSendString("Attri        |  FileSize | ShortName | LongName\r\n");
	for (cnt = 0; ;cnt++)
	{
        /* 读取目录项，索引会自动下移 */
		...
		{
			MCU_LogSendString("Path  ");
		}
		else
		{
			MCU_LogSendString("File  ");
		}
		/* 打印文件大小, 最大4G */
		MCU_LogPrintf(" %10d", (int)size);
    ...
    {
        MCU_LogPrintf("Remain Capacity Size -- %lld MB\r\n",mediaNameStr,available_bytes/1024/1024);
    }
    MCU_LogSendString("=============================================================================\r\n");
}
...
static void FileX_DriverInit(FX_MEDIA *media_ptr)
{
    ...
    {
        MCU_LogPrintf("BoardSD_Init Failed: 0X%08X\r\n",errorCodeSub);
        //异常错误
...
static void FileX_DriverRead(FX_MEDIA *media_ptr)
{
    ...
    if(errorCodeSub != ERROR_SUB_OK)  
    {
        MCU_LogPrintf("BoardSDCardReadMultiBlock Failed, Sector: %d,Count: %d,Error: 0X%08X\r\n",startSector,
...
static void FileX_DriverWrite(FX_MEDIA *media_ptr)
{
    ...
    if(errorCodeSub != ERROR_SUB_OK)  
    {
        MCU_LogPrintf("BoardSDCardWriteMultiBlock Failed, Sector: %d,Count: %d,Error: 0X%08X\r\n",startSector,
                            sectorsCount,errorCodeSub); 
        return;
    }
    media_ptr->fx_media_driver_status = FX_SUCCESS;
...
static void FileX_DriverReadBoot(FX_MEDIA *media_ptr)
{
    ...
    if(errorCodeSub != ERROR_SUB_OK)  
    {
        MCU_LogPrintf("BoardSDCardReadMultiBlock Failed, Sector: 0,Count: 1,Error: 0X%08X\r\n",errorCodeSub); 
        return;
    }
    partition_start =  0;
    ...
    if (status != 0)
    {
        MCU_LogPrintf("FileX_DriverReadBoot _fx_partition_offset_calculate Failed, Sector: 0,Count: 1,Error: 0X%08X\r\n",errorCodeSub); 
        //显示读取到的数据
        MCU_LogShowArrayBuffer("SDCard FileX Read Boot\r\n",readBuffer,BOARD_SD_CARD_SECTOR_BYTE_SIZE);
        return;
    }
...
static void FileX_DriverWriteBoot(FX_MEDIA *media_ptr)
{
    ...
    if (status != 0)
    {
        MCU_LogPrintf("FileX_DriverWriteBoot _fx_partition_offset_calculate Failed, Sector: 0,Count: 1,Error: 0X%08X\r\n",errorCodeSub);
        //显示读取到的数据
    ...
    if(partition_start != 0)
    {
        ...
        if(errorCodeSub != ERROR_SUB_OK)  
        {
            MCU_LogPrintf("BoardSDCardWriteMultiBlock Failed, Sector: %d,Count: 1,Error: 0X%08X\r\n",partition_start,errorCodeSub); 
            return;
        }
    }
    else
    {
        ...
        if(errorCodeSub != ERROR_SUB_OK)  
        {
            MCU_LogPrintf("BoardSDCardWriteMultiBlock Failed, Sector: 0,Count: 1,Error: 0X%08X\r\n",errorCodeSub); 
            return;
        }
    }
    media_ptr->fx_media_driver_status = FX_SUCCESS;
...
ERROR_SUB FileSystemSDCardFormat(void)
{
    ...
    //获取SD卡讯息
    BoardSD_GetCardInfo(&SDCardInfo);
    //动态内存申请
    if(fxAllocMemory == NULL)
    {
        fxAllocMemory = UserMemMallocWhileSuccess(FILE_X_SDCARD_ALLOC_MEM_REGION,FILE_X_SDCARD_ALLOC_MEM_LENGTH);
    }
#if(FILE_X_SDCARD_USE_EXFAT == CONFIG_FUNC_DISABLE)
    //格式化
    status = fx_media_format(&fxSDCardDiskVar, 
                              FileX_DriverSD,              //驱动函数
                              FX_NULL,                     // 可选讯息
                              (UCHAR*)fxAllocMemory,       // 工作内存
                              FILE_X_SDCARD_ALLOC_MEM_LENGTH,//工作内存长度 
                              ...
                              1);                       //每个磁道扇区数
#else
    status = fx_media_exFAT_format(&fxSDCardDiskVar, 
                              FileX_DriverSD,              //驱动函数
                              FX_NULL,                     // 可选讯息
                              (UCHAR*)fxAllocMemory,       // 工作内存
                              FILE_X_SDCARD_ALLOC_MEM_LENGTH,//工作内存长度 
    ...
    if (status != FX_SUCCESS)
    {
        UserMemFree(FILE_X_SDCARD_ALLOC_MEM_REGION,fxAllocMemory);
        fxAllocMemory = NULL;
        //错误转换
        errorCodeSub = (ERROR_SUB)(ERROR_SUB_BOARD_FILE_X_SDCARD_BASE+status);
        //日志打印
        MCU_LogPrintf("%s,fx_media_format Failed,ErrorCodeSub: 0X%08X \r\n",__func__,errorCodeSub);
        return errorCodeSub;
    }
    return errorCodeSub;
}
...
ERROR_SUB FileSystemSDCardMount(void)
{
    ...
    //挂载设备
    status =  fx_media_open(&fxSDCardDiskVar,NAME_FILEX_SDCARD,FileX_DriverSD,0,fxAllocMemory,FILE_X_SDCARD_ALLOC_MEM_LENGTH);
    if (status != FX_SUCCESS)
    {
        MCU_LogPrintf("%s,%d,FileSystemSDCardFormat Start First\r\n",__func__,__LINE__);
        //挂载失败,尝试一下格式化
        ...
        MCU_LogPrintf("%s,%d,fx_media_open Start Again\r\n",__func__,__LINE__);
        //再次尝试挂载
        status =  fx_media_open(&fxSDCardDiskVar,NAME_FILEX_SDCARD,FileX_DriverSD,0,fxAllocMemory,FILE_X_SDCARD_ALLOC_MEM_LENGTH);
        if (status != FX_SUCCESS)
        {
            ...
            errorCodeSub = (ERROR_SUB)(ERROR_SUB_BOARD_FILE_X_SDCARD_BASE+status);
            //日志打印
            MCU_LogPrintf("%s,fx_media_open Retry Failed,ErrorCode: 0X%08X \r\n",__func__,errorCodeSub);
            return errorCodeSub;
        }
    }
    //动态内存申请
    ...
    {
        fxTolerantAllocMemory = UserMemMallocWhileSuccess(FILE_X_SDCARD_ALLOC_MEM_REGION,FILE_X_SDCARD_ALLOC_TOLERANT_MEM_LENGTH);
    }
    //使能容错
    status = fx_fault_tolerant_enable(&fxSDCardDiskVar,fxTolerantAllocMemory,FILE_X_SDCARD_ALLOC_TOLERANT_MEM_LENGTH);
    if (status != FX_SUCCESS)
    {
        //卸载设备
        ...
        UserMemFree(FILE_X_SDCARD_ALLOC_MEM_REGION,fxAllocMemory);
        fxAllocMemory = NULL;
        UserMemFree(FILE_X_SDCARD_ALLOC_MEM_REGION,fxTolerantAllocMemory);
        fxTolerantAllocMemory = NULL;
        //错误转换
        errorCodeSub = (ERROR_SUB)(ERROR_SUB_BOARD_FILE_X_SDCARD_BASE+status);
        //日志打印
        MCU_LogPrintf("fx_fault_tolerant_enable Failed,ErrorCode: 0X%08X \r\n",errorCodeSub);
        return errorCodeSub;
    }
    //挂载标记
...
ERROR_SUB FileSystemSDCardUnmount(void)
{
    ...
    if (status != FX_SUCCESS)
    {
        ...
        errorCodeSub = (ERROR_SUB)(ERROR_SUB_BOARD_FILE_X_SDCARD_BASE+status);
        //日志打印
        MCU_LogPrintf("fx_media_close Failed,ErrorCode: 0X%08X \r\n",errorCodeSub);
        return errorCodeSub;
    }
    return ERROR_SUB_OK;
}
