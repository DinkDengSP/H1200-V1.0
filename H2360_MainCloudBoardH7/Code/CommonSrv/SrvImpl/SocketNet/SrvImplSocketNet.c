...
** LastEditTime  : 2023-11-07 14:02:04 +0800
************************************************************************************************/ 
...
//探针端口消息处理
static void SrvImplSocketNetProbeMsgProcess(NET_RECV_PACK* netPackPtr);
...
void SrvImplSocketNetMsgProcess(void* taskMsgPtr)
{
    ...
    {
        //探针端口消息处理
        SrvImplSocketNetProbeMsgProcess(netPackPtr);
    }
    else
    {
        //不支持的端口信息
...
static void SrvImplSocketNetProbeMsgProcess(NET_RECV_PACK* netPackPtr)
{
#if(TCP_SERVICE_PROBE_RECV_MSG_SHOW_BUF == CONFIG_FUNC_ENABLE)
    //日志打印
    ...
    BoardEC20_SendBuffer(netPackPtr->recvDatBufPtr,netPackPtr->recvDataLength);
#endif
}
...
//探针通讯端口状态机
extern void SrvImplSocketLAN8700ProbePortFSM(void);
...
void SrvImplSocketNetFSM_RunPeriod(void)
{
    ...
    //LAN8700探针通讯端口状态机
    SrvImplSocketLAN8700ProbePortFSM();
    //协议SOCKET定时检测
    uint32_t currentTimeTick = OS_TICK_TO_MS(tx_time_get());
