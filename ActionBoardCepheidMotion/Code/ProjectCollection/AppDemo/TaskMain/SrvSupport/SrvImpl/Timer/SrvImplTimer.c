...
//CAN1指令消息处理函数
extern void SrvMsgCan1TimerCmd(void* canMsgBodyPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1TimerUpload(void* canMsgBodyPtr);
//CAN2指令消息处理函数
extern void SrvMsgCan2TimerCmd(void* canMsgBodyPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2TimerUpload(void* canMsgBodyPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcTimerCmd(void* ipcMsgBodyPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcTimerUpload(void* ipcMsgBodyPtr);
...
void SrvImplTimerMsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_CMD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2TimerCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_UPLOAD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2TimerUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            SrvMsgIpcTimerCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            SrvMsgIpcTimerUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
