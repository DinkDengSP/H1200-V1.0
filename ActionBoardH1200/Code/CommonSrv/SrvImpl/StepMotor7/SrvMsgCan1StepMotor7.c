...
//电机失能
static void SrvMsgCan1StepMotor7Cmd_Disable(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机进入教学模式
static void SrvMsgCan1StepMotor7Cmd_EnterTeachMode(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机强行进入默认状态
static void SrvMsgCan1StepMotor7Cmd_ForceToDefault(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
...
static const USER_CAN_CMD_PROC_UNIT can1CmdProcStepMotor7UnitArray[] = 
{
    ...
    {CAN_MASTER_CMD_SM_RESET                ,9  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_Reset             },
    {CAN_MASTER_CMD_SM_RUN_STEPS            ,19 ,ENABLE ,SrvMsgCan1StepMotor7Cmd_RunStep           },
    {CAN_MASTER_CMD_SM_RUN_COORDINATE       ,19 ,ENABLE ,SrvMsgCan1StepMotor7Cmd_RunCoordinate     },
    {CAN_MASTER_CMD_SM_RETURN_ZERO          ,20 ,ENABLE ,SrvMsgCan1StepMotor7Cmd_ReturnZero        },
    {CAN_MASTER_CMD_SM_RUN_ALWAYS           ,5  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_RunAlways         },
    {CAN_MASTER_CMD_SM_STOP_NOW             ,0  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_StopNow           },
    {CAN_MASTER_CMD_SM_STOP_SLOW_DOWN       ,9  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_StopSlowDown      },
    ...
    {CAN_MASTER_CMD_SM_DISABLE              ,0  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_Disable           },
    {CAN_MASTER_CMD_SM_ENTER_TEACH_MODE     ,0  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_EnterTeachMode    },
    {CAN_MASTER_CMD_SM_FORCE_TO_DEFAULT     ,0  ,ENABLE ,SrvMsgCan1StepMotor7Cmd_ForceToDefault    },
};
...
static void SrvMsgCan1StepMotor7Cmd_ReturnZero(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    //执行电机回零
    SrvImplBaseStepMotor7ReturnZero(&returnZeroCmd,resultPackPtr,dataBufPtrPtr,dataBufLen);
    return;
}
...
static void SrvMsgCan1StepMotor7Cmd_Disable(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    SystemPrintf("%s\r\n",__func__);
    //调用实现API
    SrvImplBaseStepMotor7Disable(resultPackPtr,dataBufPtrPtr,dataBufLen);
    return;
}
...
static void SrvMsgCan1StepMotor7Cmd_EnterTeachMode(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    SystemPrintf("%s\r\n",__func__);
    //调用实现API
    SrvImplBaseStepMotor7EnterTeachMode(resultPackPtr,dataBufPtrPtr,dataBufLen);
    return;
}
...
static void SrvMsgCan1StepMotor7Cmd_ForceToDefault(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    SystemPrintf("%s\r\n",__func__);
    //调用实现API
    SrvImplBaseStepMotor7ForceToDefault(resultPackPtr,dataBufPtrPtr,dataBufLen);
    return;
}
/*************************************************************************************************************************************/
...
//CAN上传消息处理函数列表
static const USER_CAN_UPLOAD_PROC_UNIT can1MsgUploadProcStepMotor7UnitArray[] = 
{
...
void SrvMsgCan1StepMotor7Upload(void* can1MsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexUtil = 0;
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcStepMotor7UnitArray);indexUtil++)
    {
        if(canUploadPackPtr->uploadID == (uint32_t)(can1MsgUploadProcStepMotor7UnitArray[indexUtil].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcStepMotor7UnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(canUploadPackPtr->uploadDataBufferLength < can1MsgUploadProcStepMotor7UnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        can1MsgUploadProcStepMotor7UnitArray[indexUtil].exceptParamLengthMin,canUploadPackPtr->uploadDataBufferLength,
    ...
    //检测函数指针
    if(can1MsgUploadProcStepMotor7UnitArray[indexUtil].canUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    can1MsgUploadProcStepMotor7UnitArray[indexUtil].canUploadProcFuncPtr(canUploadPackPtr);
    //处理完成,释放内存
    if((canUploadPackPtr->uploadDataBufferPtr != NULL)&&(canUploadPackPtr->uploadDataBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_CAN,canUploadPackPtr->uploadDataBufferPtr);
