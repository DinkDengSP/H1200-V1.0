...
//CAN1指令消息处理函数
extern void ModMsgCan1NeedleWashCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void ModMsgCan1NeedleWashUpload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void ModMsgIpcNeedleWashCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void ModMsgIpcNeedleWashUpload(void* ipcReportDataPtr);
...
void ModImplNeedleWashMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleWashCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleWashUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleWashCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleWashUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
