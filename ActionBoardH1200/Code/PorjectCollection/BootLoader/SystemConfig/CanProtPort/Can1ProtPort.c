...
** LastEditTime  : 2023-10-23 11:27:20 +0800
************************************************************************************************/ 
...
ERROR_SUB Can1ProtPortSendFrame(uint32_t extendID,uint8_t msgBodyLen,uint8_t* bodyBuf)
{
    ...
    else if(msgBodyLen <= 12)
    {
        ...
        if(msgBodyLen < 12)
        {
            UserMemSet(can1ProtDataBufferTemp+msgBodyLen,0,realDataLength - msgBodyLen);
        }
    }
    else if(msgBodyLen <= 16)
    {
        realDataLength = 16;
        UserMemCopy(can1ProtDataBufferTemp,bodyBuf,msgBodyLen);
        //多余填充0
    ...
    //发送数据
    errorSub = BoardCAN1_SendMsg(extendID,CAN_ID_EXTEND,realDataLength,can1ProtDataBufferTemp);
