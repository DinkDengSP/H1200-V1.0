 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-02-02 11:56:51
...
//AckCmd数据转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgAckCmd(NET_PROT_ACK_CMD_PACK_RECV* ackCmdPackPtr)
{
...
//AckResult数据转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgAckResult(NET_PROT_ACK_RESULT_PACK_RECV* ackResultPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //透传指令转换
...
//AckUpload数据转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgAckUpload(NET_PROT_ACK_UPLOAD_PACK_RECV* ackUploadPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
#if(LOG_SHOW_SWITCH_FORWARD_SOCKET_NET == CONFIG_FUNC_ENABLE)
...
//日志信息转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgLogMsg(NET_PROT_LOG_PACK_RECV* logPackPtr)
{
#if(LOG_SHOW_SWITCH_FORWARD_SOCKET_NET == CONFIG_FUNC_ENABLE)
...
//指令信息转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgCmdMsg(NET_PROT_CMD_PACK_RECV* cmdPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //透传指令转换
...
//结果信息转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgResultMsg(NET_PROT_RESULT_PACK_RECV* resultPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //透传指令转换
...
//主动上传信息转发,基础实现是网口直接转CAN口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketNetMsgUploadMsg(NET_PROT_UPLOAD_PACK_RECV* uploadPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
#if(LOG_SHOW_SWITCH_FORWARD_SOCKET_NET == CONFIG_FUNC_ENABLE)
