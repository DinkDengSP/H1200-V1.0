...
** LastEditTime  : 2023-08-22 13:31:09 +0800
************************************************************************************************/ 
...
// NETX 驱动层保存的协议栈讯息
NETX_PORT_INFO_W5500 nxPortInfo;
// 内存池控制块
...
#pragma pack(4)
/* ARP缓存 */
ULONG w5500ArpTableBufNetX[NX_W5500_ARP_LENGTH/sizeof(ULONG)]@".dtcm_ram"; 
#pragma pack (pop) 
...
{
    uint32_t nxResult = NX_SUCCESS; // 创建内存池
    ...
    {
        MCU_LogPrintf("NetX_DriverW5500CreateMemoryPool Failed: %d \r\n", nxResult);
...
static void NetX_DriverW5500InfoReset(void)
{
    ...
    nxPortInfo.netLinkState = W5500_LINK_MODE_NONE; // 速度和连接模式
...
static void NetX_DriverW5500_CapabilitySet(NX_IP_DRIVER *nxW5500ReqPtr);
static void NetX_DriverW5500_CapabilityGet(NX_IP_DRIVER *nxW5500ReqPtr);
...
static uint8_t NetX_DriverW5500_RequestCheck(NX_IP_DRIVER *nxW5500ReqPtr)
{
    ...
    {
        // IP不匹配
        nxW5500ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL; // 错误打印
        MCU_LogPrintf("func:%s,line:%d,MultiCastJoin IP Ptr NotMatch\r\n", __func__, __LINE__);
        return 1;
    }
    // 接口匹配
    if (nxPortInfo.nxPortInfoInterfacePtr != nxW5500ReqPtr->nx_ip_driver_interface)
    {
        // 接口不匹配
        nxW5500ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL; // 错误打印
        MCU_LogPrintf("func:%s,line:%d,MultiCastJoin Interface Ptr NotMatch\r\n", __func__, __LINE__);
        return 1;
    }
    // 和内部保存指针匹配
    if (nxPortInfo.nxPortInfoIP_Ptr != &w5500NetX)
    {
        // IP不匹配
        nxW5500ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL; // 错误打印
        MCU_LogPrintf("func:%s,line:%d,MultiCastJoin IP Ptr NotLocal\r\n", __func__, __LINE__);
        return 1;
    }
    return 0;
}

#if (NETX_W5500_DRIVER_DEBUG_SHOW == CONFIG_FUNC_ENABLE)
// 打印NetX请求类型
static void NetX_DriverW5500_RequestShow(NX_IP_DRIVER *nxW5500ReqPtr)
{
    ...
    MCU_LogPrintf("NetX_DriverW5500,IPName: %s,IP: %d.%d.%d.%d , NetMask: %d.%d.%d.%d Request: ",
...
void NetX_DriverW5500(NX_IP_DRIVER *nxW5500ReqPtr)
{
    ...
    NetX_DriverW5500_RequestShow(nxW5500ReqPtr); // 处理上层协议栈对网卡的请求
    switch (nxW5500ReqPtr->nx_ip_driver_command)
    {
    case NX_LINK_INITIALIZE:
        // 驱动程序初始化
...
static void NetX_DriverW5500_Initialize(NX_IP_DRIVER *nxW5500ReqPtr)
{
    ...
    NX_INTERFACE *interface_ptr = nxW5500ReqPtr->nx_ip_driver_interface;
    ULONG ipAddress = interface_ptr->nx_interface_ip_address; // 状态匹配,必须是驱动附着状态
    ...
    {
        MCU_LogPrintf("BoardW5500_ConfigMacRawMode Failed,Code: 0X%08X\r\n", errorSubCode);
        CoreDelayMs(500);
    }
    // 检测网线连接状态
    ...
    {
        // 读取网口状态失败
        MCU_LogPrintf("BoardW5500_GetPhyState Failed : 0X%08X\r\n", errorSubCode);
        // 状态切换为已经接口附着
    ...
    {
        // 读取MAC地址失败
        MCU_LogPrintf("BoardW5500_ReadBaseSourceMacAddrReg Failed : 0X%08X\r\n", errorSubCode);
        // 状态切换为已经接口附着
        nxPortInfo.nxPortInfoState = NETX_PORT_STATE_ATTACH;
        // 回传失败
        nxW5500ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL;
    ...
    // 设置最大传输单元
    nxW5500ReqPtr->nx_ip_driver_interface->nx_interface_ip_mtu_size = NX_DRIVER_ETHERNET_MTU - NX_DRIVER_ETHERNET_FRAME_SIZE;
    // 设置MAC地址物理接口
    nxW5500ReqPtr->nx_ip_driver_interface->nx_interface_physical_address_msw = (ULONG)((macAddr[0] << 8) | (macAddr[1]));
    nxW5500ReqPtr->nx_ip_driver_interface->nx_interface_physical_address_lsw = (ULONG)((macAddr[2] << 24) | (macAddr[3] << 16) | (macAddr[4] << 8) | (macAddr[5]));
    // 设置IP与MAC地址绑定
    ...
    {
        // 创建定时器,创建的时候不使能
        tx_timer_create(&netxW5500Timer, "NetX_W5500_Timer", NetX_DriverW5500_TimerPeriod, 0, 
                            MS_TO_OS_TICK(TIME_MS_PERIOD_W5500_READ_DATA), MS_TO_OS_TICK(TIME_MS_PERIOD_W5500_READ_DATA), TX_NO_ACTIVATE);
    }
    // 状态切换为已经使能
...
static void NetX_DriverW5500_Enable(NX_IP_DRIVER *nxW5500ReqPtr)
{
    ERROR_SUB errorSubCode = ERROR_SUB_OK;
    // 状态匹配,必须是使能状态或者初始化状态
    if ((nxPortInfo.nxPortInfoState != NETX_PORT_STATE_INIT) &&(nxPortInfo.nxPortInfoState != NETX_PORT_STATE_ENABLE))
    {
        // 接口未使能
        nxW5500ReqPtr->nx_ip_driver_status = NX_NOT_SUCCESSFUL;
        return;
...
static void NetX_DriverW5500_PacketSend(NX_IP_DRIVER *nxW5500ReqPtr)
{
    ...
    *(ethernet_frame_ptr + 3) = (ip_ptr->nx_ip_arp_physical_address_lsw << 16);
...
#if ((NETX_W5500_DRIVER_DEBUG_SHOW_BUF == CONFIG_FUNC_ENABLE) || (NETX_W5500_DRIVER_DEBUG_SHOW == CONFIG_FUNC_ENABLE))
        MCU_LogPrintf("W5500 Send Data Length: %d\r\n", sendDataLength);
#endif
#if (NETX_W5500_DRIVER_DEBUG_SHOW_BUF == CONFIG_FUNC_ENABLE)
        // 显示发送
        MCU_LogShowArrayBuffer("W5500 Send DataBuffer:\r\n", packet_Copyptr->nx_packet_prepend_ptr,
    ...
    if (errorSubCode != ERROR_SUB_OK)
    {
        MCU_LogPrintf("BoardW5500_RecvOneFrameMacRaw Failed: 0X%08X\r\n", errorSubCode);
        // 释放内存
...
#if ((NETX_W5500_DRIVER_DEBUG_SHOW_BUF == CONFIG_FUNC_ENABLE) || (NETX_W5500_DRIVER_DEBUG_SHOW == CONFIG_FUNC_ENABLE))
                // 打印接收数据长度
                MCU_LogPrintf("NetX_DriverW5500_DeferredProcessing DataLength: %d\r\n", dataLength);
#endif
                // 增加一次数据包接收
...
#if (NETX_W5500_DRIVER_DEBUG_SHOW_BUF == CONFIG_FUNC_ENABLE)
                    // 显示接收
                    MCU_LogShowArrayBuffer("W5500 Recv DataBuffer:\r\n", readDataPtr, dataLength);
#endif
                }
                else
                {
                    // 没有申请到数据包
                    MCU_LogPrintf("NetX_DriverW5500_DeferredProcessing nx_packet_allocate Failed: %d\r\n", nxResult);
                    // 申请失败计数
...
{
    //获取扩展能力
    *(nxW5500ReqPtr -> nx_ip_driver_return_ptr) = 0X00000000;
    //状态成功
    nxW5500ReqPtr -> nx_ip_driver_status =  NX_SUCCESS;
}
