...
** Date          : 2023-10-16 13:33:58 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2023-10-17 11:03:32 +0800
************************************************************************************************/ 
...
void BoardMB85RS2MT_PortInit(void)
{
    ...
    //CS端口初始化
    MCU_PinInit(PIN_BOARD_MB85RS2MT_CS,MCU_PIN_MODE_OUTPUT_PP,MCU_PIN_PULL_PULLUP,MCU_PIN_SPEED_FREQ_VERY_HIGH,0);
...
ERROR_SUB BoardMB85RS2MT_EraseChip(void)
{
    ...
    //申请读写内存
    uint8_t* writeBufferPtr = MCU_MemMallocWhileSuccessAxiSram(BOARD_MB85RS2MT_BUFFER_LEN);
    uint8_t* readBufferPtr =  MCU_MemMallocWhileSuccessAxiSram(BOARD_MB85RS2MT_BUFFER_LEN);
    ...
    for(addrIndexUtil = 0;addrIndexUtil < BOARD_MB85RS2MT_SIZE;)
    {
        ...
        if(errorSub != ERROR_SUB_OK)
        {
            ...
            BoardMB85RS2MT_UnLock();
            //释放内存
            MCU_MemFreeAxiSram(writeBufferPtr);
