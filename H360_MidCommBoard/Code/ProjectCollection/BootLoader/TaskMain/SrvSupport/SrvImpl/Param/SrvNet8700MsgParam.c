...
/****************************************网口指令消息处理函数**************************************************/
//占位函数
static void SrvMsgNet8700ParamCmd_HoldPlace(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//参数单个读取        
...
static const USER_NET_CMD_PROC_UNIT net8700CmdProcParamUnitArray[] = 
{
    ...
    {(uint32_t)NET_MASTER_CMD_HOLD_PLACE                 ,0  ,ENABLE ,SrvMsgNet8700ParamCmd_HoldPlace           },
...
void SrvMsgNet8700ParamCmd(void* netCmdRecvPtr)
{
    //转换指令码
    ...
    //接收打印显示
    SystemPrintf("Param Recv Net Cmd: 0X%08X,DataLen: %d\r\n",cmdPackPtr->cmdCode,cmdPackPtr->dataBodyLength);
    //搜索匹配处理器
...
//NET上传消息处理函数列表
static const USER_NET_UPLOAD_PROC_UNIT netUploadProcParamUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)NET_MASTER_REPORT_HOLD_PLACE     ,2 ,SrvMsgNet8700ParamUpload_PlaceHolder},
};

void SrvMsgNet8700ParamUpload(void* netReportDataPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexUtil = 0;
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(netUploadProcParamUnitArray);indexUtil++)
    {
        if(netUploadPackRecvPtr->uploadID == (uint32_t)(netUploadProcParamUnitArray[indexUtil].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(netUploadProcParamUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(netUploadPackRecvPtr->paramBufferLength < netUploadProcParamUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        netUploadProcParamUnitArray[indexUtil].exceptParamLengthMin,netUploadPackRecvPtr->paramBufferLength,
    ...
    //检测函数指针
    if(netUploadProcParamUnitArray[indexUtil].netUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    netUploadProcParamUnitArray[indexUtil].netUploadProcFuncPtr(netUploadPackRecvPtr);
    //处理完成,释放内存
    if((netUploadPackRecvPtr->paramBufferPtr != NULL)&&(netUploadPackRecvPtr->paramBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_NET,netUploadPackRecvPtr->paramBufferPtr);
