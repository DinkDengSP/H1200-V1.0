 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-02-02 13:06:43
...
//AckCmd数据转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgAckCmd(PORT_CAN portCan,CAN_PROT_ACK_CMD_PACK* ackCmdPackPtr)
{
...
//AckResult数据转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgAckResult(PORT_CAN portCan,CAN_PROT_ACK_RESULT_PACK* ackResultPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //透传指令转换
    uint32_t cmdCode = 0;
...
//AckUpload数据转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgAckUpload(PORT_CAN portCan,CAN_PROT_ACK_UPLOAD_PACK* ackUploadPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
#if(LOG_SHOW_SWITCH_FORWARD_SOCKET_CAN == CONFIG_FUNC_ENABLE)
...
//日志信息转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgLogMsg(PORT_CAN portCan,CAN_PROT_LOG_PACK* logPackPtr)
{
    //日志信息打印
...
//指令信息转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgCmdMsg(PORT_CAN portCan,CAN_PROT_CMD_PACK* cmdPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //透传指令转换
    uint32_t cmdCode = 0;
...
//结果信息转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgResultMsg(PORT_CAN portCan,CAN_PROT_RESULT_PACK* resultPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //透传指令转换
    uint32_t cmdCode = 0;
...
//主动上传信息转发,基础实现是CAN口直接转网口,如果存在串口设备需要转换,那么自己重写实现
//这是一个弱定义实现,可以被外部相同定义的函数覆盖
__weak void SrvImplForwardSocketCanMsgUploadMsg(PORT_CAN portCan,CAN_PROT_UPLOAD_PACK* uploadPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
#if(LOG_SHOW_SWITCH_FORWARD_SOCKET_CAN == CONFIG_FUNC_ENABLE)
