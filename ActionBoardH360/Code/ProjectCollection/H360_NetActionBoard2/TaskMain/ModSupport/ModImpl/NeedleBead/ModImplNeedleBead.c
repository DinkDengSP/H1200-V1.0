...
//CAN1指令消息处理函数
extern void ModMsgCan1NeedleBeadCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void ModMsgCan1NeedleBeadUpload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void ModMsgIpcNeedleBeadCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void ModMsgIpcNeedleBeadUpload(void* ipcReportDataPtr);
...
void ModImplNeedleBeadMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleBeadCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleBeadUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleBeadCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleBeadUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
