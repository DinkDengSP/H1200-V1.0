...
/*************************************************************IPC指令处理处理***********************************************************/
//通讯测试
static void AppMsgIpcMsgProcCmd_CommTest(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);

//IPC指令处理函数列表
static const USER_IPC_CMD_PROC_UNIT ipcMsgProcMsgCmdProcUnitArray[] = 
{
    //顺序是 指令码 预期参数最小长度  指令处理函数
    {(uint32_t)0XFFFFFFFF     ,0    ,AppMsgIpcMsgProcCmd_CommTest  },
};


//IPC指令消息处理函数
void AppMsgIpcMsgProcCmd(void* ipcMsgBodyPtr)
{
    //转换指令码
    ...
    for(indexMsgProc = 0; indexMsgProc < DIM_ARRAY_ELEMENT_COUNT(ipcMsgProcMsgCmdProcUnitArray);indexMsgProc++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcMsgProcMsgCmdProcUnitArray[indexMsgProc].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexMsgProc == DIM_ARRAY_ELEMENT_COUNT(ipcMsgProcMsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcMsgProcMsgCmdProcUnitArray[indexMsgProc].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcMsgProcMsgCmdProcUnitArray[indexMsgProc].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcMsgProcMsgCmdProcUnitArray[indexMsgProc].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcMsgProcMsgCmdProcUnitArray[indexMsgProc].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
/*************************************************************IPC上报讯息处理**********************************************************/
//占位函数
static void AppMsgIpcMsgProcUpload_PlaceHolder(IPC_UPLOAD_DATA* ipcUploadDataPtr);

//IPC上传消息处理函数列表
static const USER_IPC_UPLOAD_PROC_UNIT ipcMsgProcMsgUploadProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)0XFFFFFFFF     ,2 ,AppMsgIpcMsgProcUpload_PlaceHolder},
};


//IPC上传消息处理函数
void AppMsgIpcMsgProcUpload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexMsgProc = 0;
    for(indexMsgProc = 0; indexMsgProc < DIM_ARRAY_ELEMENT_COUNT(ipcMsgProcMsgUploadProcUnitArray);indexMsgProc++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcMsgProcMsgUploadProcUnitArray[indexMsgProc].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexMsgProc == DIM_ARRAY_ELEMENT_COUNT(ipcMsgProcMsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcMsgProcMsgUploadProcUnitArray[indexMsgProc].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcMsgProcMsgUploadProcUnitArray[indexMsgProc].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcMsgProcMsgUploadProcUnitArray[indexMsgProc].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcMsgProcMsgUploadProcUnitArray[indexMsgProc].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
