...
/*******************************************CAN指令处理函数列表*************************************************/
//占位函数
static void SrvMsgCan2StepMotor7Cmd_PlaceHolder(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机复位
static void SrvMsgCan2StepMotor7Cmd_Reset(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机走步数
static void SrvMsgCan2StepMotor7Cmd_RunStep(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机走坐标
static void SrvMsgCan2StepMotor7Cmd_RunCoordinate(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机回零
static void SrvMsgCan2StepMotor7Cmd_ReturnZero(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机持续运转
static void SrvMsgCan2StepMotor7Cmd_RunAlways(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机立即停止
static void SrvMsgCan2StepMotor7Cmd_StopNow(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//电机减速停止
static void SrvMsgCan2StepMotor7Cmd_StopSlowDown(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//获取电机当前坐标
static void SrvMsgCan2StepMotor7Cmd_GetCoordinate(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//获取当前原点传感器状态
static void SrvMsgCan2StepMotor7Cmd_ReadOriginSensor(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);

//CAN指令处理函数列表
static const USER_CAN_CMD_PROC_UNIT can2CmdProcStepMotor7UnitArray[] = 
{
    //顺序是 指令码 预期参数最小长度 是否需要结果返回  错误是否独立上报 指令处理函数
    {CAN_MASTER_CMD_PLACE_HOLDER            ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_PlaceHolder  },
    {CAN_MASTER_CMD_SM_RESET                ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_Reset             },
    {CAN_MASTER_CMD_SM_RUN_STEPS            ,4  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_RunStep           },
    {CAN_MASTER_CMD_SM_RUN_COORDINATE       ,4  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_RunCoordinate     },
    {CAN_MASTER_CMD_SM_RETURN_ZERO          ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_ReturnZero        },
    {CAN_MASTER_CMD_SM_RUN_ALWAYS           ,1  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_RunAlways         },
    {CAN_MASTER_CMD_SM_STOP_NOW             ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_StopNow           },
    {CAN_MASTER_CMD_SM_STOP_SLOW_DOWN       ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_StopSlowDown      },
    {CAN_MASTER_CMD_SM_GET_COORDINATE       ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_GetCoordinate     },
    {CAN_MASTER_CMD_SM_READ_ORIGIN_SENSOR   ,0  ,ENABLE ,SrvMsgCan2StepMotor7Cmd_ReadOriginSensor  },
};


//CAN指令消息处理函数
void SrvMsgCan2StepMotor7Cmd(void* can2MsgBodyPtr)
{
    //转换指令码
    ...
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(can2CmdProcStepMotor7UnitArray);indexUtil++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can2CmdProcStepMotor7UnitArray[indexUtil].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(can2CmdProcStepMotor7UnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can2CmdProcStepMotor7UnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can2CmdProcStepMotor7UnitArray[indexUtil].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can2CmdProcStepMotor7UnitArray[indexUtil].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can2CmdProcStepMotor7UnitArray[indexUtil].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can2CmdProcStepMotor7UnitArray[indexUtil].needResultReply == ENABLE)
    {
        //需要结果回传
...
/*************************************************************CAN上传参数处理***********************************************************/
//占位函数
static void SrvMsgCan2StepMotor7Upload_PlaceHolder(CAN_PROT_UPLOAD_PACK* canUploadPackPtr);

//CAN上传消息处理函数列表
static const USER_CAN_UPLOAD_PROC_UNIT can2MsgUploadProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {CAN_MASTER_REPORT_ID_HOLD_PLACE     ,2 ,SrvMsgCan2StepMotor7Upload_PlaceHolder},
};

//CAN上传消息处理函数
void SrvMsgCan2StepMotor7Upload(void* can2MsgBodyPtr)
{
