 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-01-24 14:22:29
...
//电机失能
static void SrvMsgIpcStepMotor9Cmd_Disable(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);
//电机进入教学模式
static void SrvMsgIpcStepMotor9Cmd_EnterTeachMode(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);
//电机强行进入默认状态
static void SrvMsgIpcStepMotor9Cmd_ForceToDefault(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);
...
static const USER_IPC_CMD_PROC_UNIT ipcStepMotor9MsgCmdProcUnitArray[] = 
{
    ...
    {(uint32_t)0X00000009     ,0    ,SrvMsgIpcStepMotor9Cmd_Disable             },
    {(uint32_t)0X0000000A     ,0    ,SrvMsgIpcStepMotor9Cmd_EnterTeachMode      },
    {(uint32_t)0X0000000B     ,0    ,SrvMsgIpcStepMotor9Cmd_ForceToDefault      },
};
...
void SrvMsgIpcStepMotor9Cmd(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexStepMotor9 = 0;
    for(indexStepMotor9 = 0; indexStepMotor9 < DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor9MsgCmdProcUnitArray);indexStepMotor9++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcStepMotor9MsgCmdProcUnitArray[indexStepMotor9].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexStepMotor9 == DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor9MsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcStepMotor9MsgCmdProcUnitArray[indexStepMotor9].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcStepMotor9MsgCmdProcUnitArray[indexStepMotor9].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcStepMotor9MsgCmdProcUnitArray[indexStepMotor9].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcStepMotor9MsgCmdProcUnitArray[indexStepMotor9].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
static void SrvMsgIpcStepMotor9Cmd_ReturnZero(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr)
{
    ...
    channelCachePtr->returnDataBufferLength = 0;
    //调用API
    SrvImplBaseStepMotor9ReturnZero(&returnZeroCmd,resultPackPtr,&returnBufferPtr,&returnBufferLength);
    //对结果进行处理
    if(resultPackPtr->resultCode == PROT_RESULT_SUCCESS)
    {
        if((returnBufferPtr != NULL)&&(returnBufferLength != 0))
...
static void SrvMsgIpcStepMotor9Cmd_Disable(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr)
{
    ...
    channelCachePtr->returnDataBufferLength = 0;
    //调用API实现
    SrvImplBaseStepMotor9Disable(resultPackPtr,&returnBufferPtr,&returnBufferLength);
    //对结果进行处理
    if(resultPackPtr->resultCode == PROT_RESULT_SUCCESS)
    {
        if((returnBufferPtr != NULL)&&(returnBufferLength != 0))
...
static void SrvMsgIpcStepMotor9Cmd_EnterTeachMode(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr)
{
    ...
    channelCachePtr->returnDataBufferLength = 0;
    //调用API实现
    SrvImplBaseStepMotor9EnterTeachMode(resultPackPtr,&returnBufferPtr,&returnBufferLength);
    //对结果进行处理
    if(resultPackPtr->resultCode == PROT_RESULT_SUCCESS)
    {
        if((returnBufferPtr != NULL)&&(returnBufferLength != 0))
...
static void SrvMsgIpcStepMotor9Cmd_ForceToDefault(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr)
{
    ...
    channelCachePtr->returnDataBufferLength = 0;
    //调用API实现
    SrvImplBaseStepMotor9ForceToDefault(resultPackPtr,&returnBufferPtr,&returnBufferLength);
    //对结果进行处理
    if(resultPackPtr->resultCode == PROT_RESULT_SUCCESS)
    {
        if((returnBufferPtr != NULL)&&(returnBufferLength != 0))
...
void SrvMsgIpcStepMotor9Upload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexStepMotor9 = 0;
    for(indexStepMotor9 = 0; indexStepMotor9 < DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor9MsgUploadProcUnitArray);indexStepMotor9++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcStepMotor9MsgUploadProcUnitArray[indexStepMotor9].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexStepMotor9 == DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor9MsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcStepMotor9MsgUploadProcUnitArray[indexStepMotor9].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcStepMotor9MsgUploadProcUnitArray[indexStepMotor9].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcStepMotor9MsgUploadProcUnitArray[indexStepMotor9].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcStepMotor9MsgUploadProcUnitArray[indexStepMotor9].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
