...
{
    stepMotorVar[STEP_MOTOR_NO4]->var.motorMainState = mainState;
    stepMotorVar[STEP_MOTOR_NO4]->var.motorSubState = subState;
}
...
void IPC_SrvStepMotor4EmergencyStop(void)
{
    ...
    stepMotorVar[STEP_MOTOR_NO4]->var.motorSubState = STEP_MOTOR_STOP_SUB_EMERGENCY_CMD;
    //步进电机写入急停
    BoardCPLD_StepMotorEmergencyStop(BOARD_CPLD_MOTOR4);
}

//修改电机坐标
void IPC_SrvStepMotor4ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVar[STEP_MOTOR_NO4]->var.currentCoordinate = motorCurrentCoordinate;
}

//获取电机实时坐标
int32_t IPC_SrvStepMotor4GetRealTimeCoordinate(void)
{
    return stepMotorVar[STEP_MOTOR_NO4]->var.currentCoordinate;
}
