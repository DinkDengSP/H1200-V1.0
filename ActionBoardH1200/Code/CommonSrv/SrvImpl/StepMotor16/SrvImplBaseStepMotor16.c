                                        ...
                                        uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = UserMemMallocWhileSuccess(MEM_REGION_SRV_TASK,*dataBufLen);
    ...
    //读取当前坐标
    StepMotorReadMotorPosAndEncoderPos(STEP_MOTOR_NO16,&motorCurrentCoordinate,&motorCurrentEncoder);
    //读取电机传感器状态
    MOTOR_SENSOR_SET motorSensorStates;
    StepMotorReadSensorState(STEP_MOTOR_NO16,&motorSensorStates);
    //数据写入返回
...
void SrvImplBaseStepMotor16StopNow(SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    ...
    CoreUtilSetSystemResultPackNoError(resultPackPtr);
    //立即停止
    errorSub = StepMotorRunAlwaysStopImmediate(STEP_MOTOR_NO16);
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_ACT_STEP_MOTOR,ERROR_LEVEL_ERROR,
                                        ...
                                        uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = UserMemMallocWhileSuccess(MEM_REGION_SRV_TASK,*dataBufLen);
    ...
    {
        //慢速
        errorSub = StepMotorRunAlwaysDecSlow(STEP_MOTOR_NO16,timeOutMs);
    }
    else
    {
        //快速
        errorSub = StepMotorRunAlwaysDecFast(STEP_MOTOR_NO16,slowDownSteps,timeOutMs);
    }
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
...
void SrvImplBaseStepMotor16Disable(SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    ...
    CoreUtilSetSystemResultPackNoError(resultPackPtr);
    //立即停止
    errorSub = StepMotorMainDisable(STEP_MOTOR_NO16);
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_ACT_STEP_MOTOR,ERROR_LEVEL_ERROR,
...
void SrvImplBaseStepMotor16EnterTeachMode(SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    ...
    CoreUtilSetSystemResultPackNoError(resultPackPtr);
    //立即停止
    errorSub = StepMotorMainEnterTeachingMode(STEP_MOTOR_NO16);
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_ACT_STEP_MOTOR,ERROR_LEVEL_ERROR,
...
void SrvImplBaseStepMotor16ForceToDefault(SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    ...
    CoreUtilSetSystemResultPackNoError(resultPackPtr);
    //立即停止
    errorSub = StepMotorMainForceToDefaultState(STEP_MOTOR_NO16);
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_ACT_STEP_MOTOR,ERROR_LEVEL_ERROR,
