 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-01-31 14:46:37
...
void SrvImplBaseFlashIapServiceUpdata(BOARD_IAP_RECODER* iapRecordPtr)
{
    ...
    //读取SERVICE
    errorSubCode = BoardIapReadSrvBinFile(0,srvBinArrayPtr,iapRecordPtr->iapFileTotalBytes);
    if(errorSubCode != ERROR_SUB_OK)
    {
        SystemPrintf("%s,BoardIapReadSrvBinFile Failed,ErrorSub: 0X%08X \r\n",__func__,errorSubCode);
        //清除IAP标记,设置BOOT模式进行下次升级
        errorSubCode = BoardIapWriteServiceRecordDefault();
        if(errorSubCode != ERROR_SUB_OK)
        {
            SystemPrintf("%s,BoardIapWriteServiceRecordDefault Failed,ErrorSub: 0X%08X \r\n",__func__,errorSubCode);
        }
        //设置BOOT模式,下次继续升级
        errorSubCode = BoardIapSetRunningModeBoot();
        if(errorSubCode != ERROR_SUB_OK)
        {
            SystemPrintf("%s,BoardIapSetRunningModeBoot Failed,ErrorSub: 0X%08X \r\n",__func__,errorSubCode);
        }
        //释放内存
    ...
    //擦除Flash
    errorSubCode = MCU_FlashEraseRangeWithCheck(MCU_FLASH_SRV_MAIN,iapRecordPtr->iapFileTotalBytes);
    if(errorSubCode != ERROR_SUB_OK)
    {
    ...
    do
    {
        retryCount++;
        //CRC匹配,开始写入FLASH
        errorSubCode = MCU_FlashWrite(MCU_FLASH_SRV_MAIN,srvBinArrayPtr,iapRecordPtr->iapFileTotalBytes);
        if(errorSubCode != ERROR_SUB_OK)
        {
            //结束循环,下一次
            continue;
        ...
        CoreDelayMs(100);
        //比较写入是否正确
        flashCmpResult = MCU_FlashCompare(MCU_FLASH_SRV_MAIN,srvBinArrayPtr,iapRecordPtr->iapFileTotalBytes);
    ...
    UserMemFree(MEM_SDRAM1,srvBinArrayPtr);
    //清除成功,跳转到APP
    errorSubCode = BoardIapJumpApp();
    if(errorSubCode != ERROR_SUB_OK)
    {
        SystemPrintf("%s,BoardIapJumpApp Failed,ErrorSub: 0X%08X \r\n",__func__,errorSubCode);
    }
    return;
}
...
void SrvImplBaseFlashIapStartMsg(BOARD_IAP_FILE_INFO* iapInfoPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    if(boardIapFileInfo.boardIapMode == BOARD_IAP_MODE_BOOT)
    {
        if(boardIapFileInfo.totalFileBytes > LENGTH_BOARD_IAP_BOOT_BIN_FILE)
        {
            ...
            //设置错误
            CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_DMC_FLASH_IAP_START,
                                        ERROR_LEVEL_ERROR,ERROR_SUB_BOARD_IAP_BOOT_FILE_BOOT_BYTES_LIMIT);
            return;
        }
        //清除BOOT升级文件信息
