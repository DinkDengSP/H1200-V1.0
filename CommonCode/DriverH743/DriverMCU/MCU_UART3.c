...
//传输正在进行标志
static MCU_UART_TRANS_STATE mcuUart3TransState = MCU_UART_TRANS_IDLE;
#endif
//中断发送,需要使用的数据块
...
#pragma pack(4)
//用于当前发送的数据块
static uint8_t blockBufferWithUart3SendCurrent[LENGTH_UART3_BUFFER_WITH_BLOCK]@".d2_sram2_dma" = {0};
#pragma pack (pop)
#endif
...
//接收缓存
static UART_RECV_PACK uart3RecvPackArray[COUNT_UART3_RECV_BLOCK];
//接收缓存
...
//当前接收对象
static uint8_t currentBufferWithUart3Recv[LENGTH_UART3_RECV_SINGLE_BLOCK]@".d2_sram2_dma" = {0};
#endif  
...
static void MCU_UART3_BaseInit(uint32_t baud, MCU_UART_LENGTH length, MCU_UART_STOPBIT stopBit,MCU_UART_CHECK_MODE checkMode, MCU_UART_HARD_CONTROL hardWareControl)
{
    ...
    UART3_Handler.Init.OverSampling = MCU_UART3_OVERSAMPLING_MODE;
    //采样值的位数,是否打开一个采样位方法,打开的话,将禁止噪声检测标记
    UART3_Handler.Init.OneBitSampling = MCU_UART3_ONE_BIT_SAMPLE;
    //关闭FIFO模式
    UART3_Handler.FifoMode = UART_FIFOMODE_DISABLE;
    //高级配置,其中可以设置端口反转和电平反转,根据需要设计自己的高级特性相或
    uint32_t advFeatureInit = UART_ADVFEATURE_NO_INIT;
    //是否关闭ORE过载标记
#if(MCU_UART3_DISABLE_ORE != 0)
    advFeatureInit |= UART_ADVFEATURE_RXOVERRUNDISABLE_INIT;
#endif
    UART3_Handler.AdvancedInit.AdvFeatureInit = advFeatureInit;
    //底层初始化
    MCU_UART3_MspInit();
    //根据配置初始化串口
...
#if((MCU_UART3_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART3_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
    //数据结构初始化
    circularByteQueueUart3SendPtr = ADT_CircularByteQueueCreate(LENGTH_UART3_BUFFER_WITH_SEND,MCU_MemMallocAxiSram,MCU_MemFreeAxiSram,
...
void MCU_UART3_TransDMA_Init(void)
{
    ...
    //参数配置
    /*DMA数据流请求*/	
    UART3_TxDMA_Handler.Init.Request = DMA_REQUEST_USART3_TX; 
    /*方向：从内存到外设*/		
...
void MCU_UART3_RecvDataInit(void)
{
    ...
    //初始化接收队列
    circularPtrQueueUart3RecvIdleBlockPtr = ADT_CircularPtrQueueCreate(COUNT_UART3_RECV_BLOCK,MCU_MemMallocAxiSram,MCU_MemFreeAxiSram,
    ...
    {
        uart3RecvPackArray[indexUtil].packSrc = UART_PACK_SRC_3;
        uart3RecvPackArray[indexUtil].recvDataBufferPtr = (uint8_t*)(blockBufferWithUart3Recv[indexUtil]);
        uart3RecvPackArray[indexUtil].recvDataLength = 0;
    }
    //队列导入
    for(indexUtil = 0;indexUtil < COUNT_UART3_RECV_BLOCK;indexUtil++)
    {
        ADT_CircularPtrQueueWrite(circularPtrQueueUart3RecvIdleBlockPtr,(void*)(&(uart3RecvPackArray[indexUtil])));
    }
    //清除当前接收队列
...
void MCU_UART3_RecvDMA_Init(void)
{
    ...
    //参数配置
    /*DMA接收数据流请求*/	
    UART3_RxDMA_Handler.Init.Request = DMA_REQUEST_USART3_RX; 
    /*方向：从外设到内存*/		
...
//串口接收到的数据释放
void MCU_UART3_RecvDatRelease(UART_RECV_PACK* uartRecvPackPtr)
{
    asm("nop");
#if((MCU_UART3_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART3_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
    ...
    UserMemClear((void*)(uartRecvPackPtr->recvDataBufferPtr),LENGTH_UART3_RECV_SINGLE_BLOCK);
    //数据指针写回
    ADT_CircularPtrQueueWrite(circularPtrQueueUart3RecvIdleBlockPtr,uartRecvPackPtr);
#endif
}
...
//串口回调函数,接收完成中断
void MCU_UART3_RxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    asm("nop");
#if((MCU_UART3_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART3_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
    ...
    else
    {
        ...
        recvDispatchPtr->recvDataLength = LENGTH_UART3_RECV_SINGLE_BLOCK;
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->recvDataBufferPtr),(void*)currentBufferWithUart3Recv,LENGTH_UART3_RECV_SINGLE_BLOCK);
        //清空接收
        UserMemClear((void*)currentBufferWithUart3Recv,LENGTH_UART3_RECV_SINGLE_BLOCK);
#if(MCU_UART3_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
        //该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
...
//串口回调函数,发送完成中断
void MCU_UART3_TxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    asm("nop");
#if((MCU_UART3_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART3_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
...
//串口回调函数,错误中断
void MCU_UART3_ErrorCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    //关闭接收
...
//串口回调函数,接收事件中断
void MCU_UART3_RxEventCallBack(UART_HandleTypeDef* uartHandlerPtr,uint16_t dataLength)
{
#if(MCU_UART3_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
    ...
    else
    {
        ...
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->recvDataBufferPtr),(void*)currentBufferWithUart3Recv,dataLength);
        //清空接收
        UserMemClear((void*)currentBufferWithUart3Recv,LENGTH_UART3_RECV_SINGLE_BLOCK);
        //打开空闲接收
        HAL_UARTEx_ReceiveToIdle_IT(&UART3_Handler,(uint8_t *)(currentBufferWithUart3Recv), LENGTH_UART3_RECV_SINGLE_BLOCK);
