...
//串口回调函数
static MCU_UART_CALLBACK uart6CallBackSet = {0};
...
//循环队列
static ADT_CIRCULAR_BYTE_QUEUE_T circularByteQueueUart6SendPtr = NULL;
//传输正在进行标志
static MCU_UART_TRANS_STATE mcuUart6TransState = MCU_UART_TRANS_IDLE;
#endif
//中断发送,需要使用的数据块
...
#pragma pack(4)
//用于当前发送的数据块
static uint8_t blockBufferWithUart6SendCurrent[LENGTH_UART6_BUFFER_WITH_BLOCK]@".d3_sram4_dma" = {0};
#pragma pack (pop)
#endif
...
//循环队列,保存数据指针
static ADT_CIRCULAR_PTR_QUEUE_T circularPtrQueueUart6RecvIdleBlockPtr = NULL;
//接收缓存
...
//当前接收对象
static uint8_t currentBufferWithUart6Recv[LENGTH_UART6_RECV_SINGLE_BLOCK]@".d3_sram4_dma" = {0};
#endif  
...
{
#if((MCU_UART6_TRANS_MODE_USER == MCU_UART_TRANS_MODE_TXRX)||(MCU_UART6_TRANS_MODE_USER == MCU_UART_TRANS_MODE_TX))
    MCU_PinInit(UART6_PIN_TX,MCU_PIN_MODE_AF_PP,MCU_PIN_PULL_PULLUP,MCU_PIN_SPEED_FREQ_HIGH,GPIO_AF7_USART6);
#endif
#if((MCU_UART6_TRANS_MODE_USER == MCU_UART_TRANS_MODE_TXRX)||(MCU_UART6_TRANS_MODE_USER == MCU_UART_TRANS_MODE_RX))
...
//串口回调函数,接收完成中断
static void MCU_UART6_RxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr);
//串口回调函数,发送完成中断
static void MCU_UART6_TxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr);
//串口回调函数,错误中断
static void MCU_UART6_ErrorCallBack(UART_HandleTypeDef* uartHandlerPtr);
//串口回调函数,接收事件中断
static void MCU_UART6_RxEventCallBack(UART_HandleTypeDef* uartHandlerPtr,uint16_t dataLength);
...
static void MCU_UART6_BaseInit(uint32_t baud, MCU_UART_LENGTH length, MCU_UART_STOPBIT stopBit,MCU_UART_CHECK_MODE checkMode, MCU_UART_HARD_CONTROL hardWareControl)
{
    ...
    //是否关闭ORE过载标记
#if(MCU_UART6_DISABLE_ORE == CONFIG_FUNC_ENABLE)
...
#if((MCU_UART6_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART6_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
    //数据结构初始化
    circularByteQueueUart6SendPtr = ADT_CircularByteQueueCreate(LENGTH_UART6_BUFFER_WITH_SEND,MCU_MemMallocAxiSram,MCU_MemFreeAxiSram,
...
void MCU_UART6_RecvDataInit(void)
{
    ...
    //初始化接收队列
    circularPtrQueueUart6RecvIdleBlockPtr = ADT_CircularPtrQueueCreate(COUNT_UART6_RECV_BLOCK,MCU_MemMallocAxiSram,MCU_MemFreeAxiSram,
    ...
    {
        uart6RecvPackArray[indexUtil].packSrc = UART_PACK_SRC_6;
    ...
    for(indexUtil = 0;indexUtil < COUNT_UART6_RECV_BLOCK;indexUtil++)
    {
        ADT_CircularPtrQueueWrite(circularPtrQueueUart6RecvIdleBlockPtr,(void*)(&(uart6RecvPackArray[indexUtil])));
    }
    //清除当前接收队列
...
{
     //基础串口初始化
    MCU_UART6_BaseInit(baud,length,stopBit,checkMode,hardWareControl);
    //相关数据初始化
    ...
    MCU_UART6_StartRecv();
    //中断回调关联
    UART6_Handler.TxCpltCallback = MCU_UART6_TxCpltCallBack;
    UART6_Handler.RxCpltCallback = MCU_UART6_RxCpltCallBack;
    UART6_Handler.ErrorCallback = MCU_UART6_ErrorCallBack;
    UART6_Handler.RxEventCallback = MCU_UART6_RxEventCallBack;
    //清空回调函数
    uart6CallBackSet.recvIntCallBack = NULL;
    uart6CallBackSet.sendDataOverCallBack = NULL;
    uart6CallBackSet.sendDataStartCallBack = NULL;
}

//配置串口回调函数
void MCU_UART6_ConfigCallBack(MCU_UartRecvIntProcFunc recvCallBack,MCU_UartSendDataStartProcFunc sendStartCallBack,
                                MCU_UartSendDataOverProcFunc sendOverCallBack)
{
    uart6CallBackSet.recvIntCallBack = recvCallBack;
    uart6CallBackSet.sendDataStartCallBack = sendStartCallBack;
    uart6CallBackSet.sendDataOverCallBack = sendOverCallBack;
}
...
void MCU_UART6_SendBufferPool(uint8_t* bufferStartPtr,uint16_t sendLength)
{
    ...
    MCU_UART6_GetLock();
    //发送前回调,针对485优化
    if(uart6CallBackSet.sendDataStartCallBack != NULL)
    {
        uart6CallBackSet.sendDataStartCallBack();
    }
    //阻塞发送
    ...
    //发送后回调,针对485回调
    if(uart6CallBackSet.sendDataOverCallBack != NULL)
    {
        uart6CallBackSet.sendDataOverCallBack();
    }
    //释放信号量
    MCU_UART6_ReleaseLock();
}
#endif
...
static ErrorStatus MCU_UART6_WriteBufferToQueue(uint8_t* bufferStartPtr,uint16_t sendLength)
{
    ...
    do
    {
        ...
        if(ADT_CircularByteQueueGetCurrentEmptySize(circularByteQueueUart6SendPtr) > 0)
        {
            //写入循环队列
            ADT_CircularByteQueueWriteBuffer(circularByteQueueUart6SendPtr,(bufferStartPtr+currentBufferHadSendLength),currentBufferWaitSendLength,&realWriteDataCount);
        ...
        {
            //从循环队列读取指定长度数据
            ADT_CircularByteQueueReadBuffer(circularByteQueueUart6SendPtr,blockBufferWithUart6SendCurrent,LENGTH_UART6_BUFFER_WITH_BLOCK,&realWriteDataCount);
            //数据长度大于0
            if(realWriteDataCount > 0)
            {
                ...
                if(halState != HAL_OK)
                {
                    ...
                    mcuUart6TransState = MCU_UART_TRANS_IDLE;
                    //清空数据
                    ADT_CircularByteQueueClear(circularByteQueueUart6SendPtr);
                    //发送后回调,针对485回调
                    if(uart6CallBackSet.sendDataOverCallBack != NULL)
                    {
                        uart6CallBackSet.sendDataOverCallBack();
...
//是否异步发送
#if(UART6_TRANS_FUNC_SYNC == CONFIG_FUNC_ENABLE) 
    //等待发送完成
...
void MCU_UART6_RecvDatRelease(UART_RECV_PACK* uartRecvPackPtr)
{
    ...
    //数据指针写回
    ADT_CircularPtrQueueWrite(circularPtrQueueUart6RecvIdleBlockPtr,uartRecvPackPtr);
#endif
}
...
//串口回调函数,接收完成中断
void MCU_UART6_RxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    asm("nop");
#if((MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
    if(uart6CallBackSet.recvIntCallBack == NULL)
    {
        //没有转发函数,清空接收
        UserMemClear((void*)currentBufferWithUart6Recv,LENGTH_UART6_RECV_SINGLE_BLOCK);
#if(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
    ...
    //从队列中拿出一个指针做接收转发
    UART_RECV_PACK* recvDispatchPtr = ADT_CircularPtrQueueGet(circularPtrQueueUart6RecvIdleBlockPtr);
    ...
    {
        //配置接收参数
        recvDispatchPtr->packSrc = UART_PACK_SRC_6;
...
#endif   
        //数据转发
        if(ERROR == uart6CallBackSet.recvIntCallBack(recvDispatchPtr))
        {
            //转发失败,将数据放回去
...
//串口回调函数,发送完成中断
void MCU_UART6_TxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    asm("nop");
#if((MCU_UART6_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART6_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
    ...
    //队列现在没数据
    if(ADT_CircularByteQueueGetCurrentSize(circularByteQueueUart6SendPtr) == 0)
    {
        //设置发送结束
        mcuUart6TransState = MCU_UART_TRANS_IDLE; 
        //发送后回调,针对485优化
    ...
    //从队列中取数据
    ADT_CircularByteQueueReadBuffer(circularByteQueueUart6SendPtr,blockBufferWithUart6SendCurrent,LENGTH_UART6_BUFFER_WITH_BLOCK,&transPackLength);
    //缓冲区里面有数据
...
//串口回调函数,错误中断
void MCU_UART6_ErrorCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    //关闭接收
...
//串口回调函数,接收事件中断
void MCU_UART6_RxEventCallBack(UART_HandleTypeDef* uartHandlerPtr,uint16_t dataLength)
{
#if(MCU_UART6_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
    if((dataLength == 0)||(uart6CallBackSet.recvIntCallBack == NULL))
    {
        //清空接收
        UserMemClear((void*)currentBufferWithUart6Recv,LENGTH_UART6_RECV_SINGLE_BLOCK);
        //打开空闲接收
