 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-02-02 14:12:45
...
/****************************************网口指令消息处理函数**************************************************/
//占位函数
static void SrvMsgNet8700FlashIapCmd_PlaceHolder(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//在线升级准备
...
static const USER_NET_CMD_PROC_UNIT net8700CmdProcFlashIapUnitArray[] = 
{
    ...
    {(uint32_t)NET_MASTER_CMD_HOLD_PLACE            ,0  ,ENABLE ,SrvMsgNet8700FlashIapCmd_PlaceHolder   },
...
void SrvMsgNet8700FlashIapCmd(void* net8700CmdRecvPtr)
{
    //转换指令码
    ...
    //接收打印显示
    SystemPrintf("FlashIap Recv Net8700 Cmd: 0X%08X,DataLen: %d\r\n",cmdPackPtr->cmdCode,cmdPackPtr->dataBodyLength);
    //搜索匹配处理器
...
static void SrvMsgNet8700FlashIapCmd_DataCheck(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    {
        //擦除Flash
        errorSub = MCU_FlashEraseRangeWithCheck(MCU_FLASH_SRV_BOOT,boardIapFileInfo.totalFileBytes);
        if(errorSub != ERROR_SUB_OK)
        {
            //写入flash失败
            SystemPrintf("%s ,%d,MCU_FlashEraseRangeWithCheck ErrorSub: 0X%08X\r\n",__func__,__LINE__,errorSub);
            //释放内存
            UserMemFree(MEM_SDRAM1,iapFileBinArray);
            //清空标记
...
//NET上传消息处理函数列表
static const USER_NET_UPLOAD_PROC_UNIT netUploadFlashIapProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)NET_MASTER_REPORT_HOLD_PLACE     ,2 ,SrvMsgNet8700FlashIapUpload_PlaceHolder},
};

void SrvMsgNet8700FlashIapUpload(void* net8700ReportDataPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexUtil = 0;
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(netUploadFlashIapProcUnitArray);indexUtil++)
    {
        if(netUploadPackRecvPtr->uploadID == (uint32_t)(netUploadFlashIapProcUnitArray[indexUtil].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(netUploadFlashIapProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(netUploadPackRecvPtr->paramBufferLength < netUploadFlashIapProcUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        netUploadFlashIapProcUnitArray[indexUtil].exceptParamLengthMin,netUploadPackRecvPtr->paramBufferLength,
    ...
    //检测函数指针
    if(netUploadFlashIapProcUnitArray[indexUtil].netUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    netUploadFlashIapProcUnitArray[indexUtil].netUploadProcFuncPtr(netUploadPackRecvPtr);
    //处理完成,释放内存
    if((netUploadPackRecvPtr->paramBufferPtr != NULL)&&(netUploadPackRecvPtr->paramBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_NET,netUploadPackRecvPtr->paramBufferPtr);
