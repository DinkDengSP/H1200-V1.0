...
static void MCU_SPI2_BaseConfig(MCU_SPI_123_CLK speed,MCU_SPI_CPOL cpol,MCU_SPI_CPHA cpha)
{
    ...
    SPI2_Handler.Init.MasterInterDataIdleness = SPI_MASTER_INTERDATA_IDLENESS_02CYCLE;
...
#pragma pack(4)
//用于DMA发送数据快
static uint8_t bufferWithSpi1Send[LENGTH_BYTES_SPI2_DMA_BUF]@".axi_dma_sram" = {0};
//用于DMA接收数据块
static uint8_t bufferWithSpi1Recv[LENGTH_BYTES_SPI2_DMA_BUF]@".axi_dma_sram" = {0};
#pragma pack (pop)
...
static void MCU_SPI2_DMA_Init(void)
{
    ...
    SPI2_DMA_TX_Handler.Init.MemBurst            = DMA_MBURST_SINGLE;	
    SPI2_DMA_TX_Handler.Init.PeriphBurst         = DMA_PBURST_SINGLE;	
    ...
    halState = HAL_DMA_Init(&SPI2_DMA_TX_Handler);
    if(halState != HAL_OK)
    {
        MCU_LogShowHalState(halState,SPI2_DMA_TX_Handler.ErrorCode);
        while(1);
    }
    //发送DMA中断设定
    ...
    halState = HAL_DMA_Init(&SPI2_DMA_RX_Handler);    
    if(halState != HAL_OK)
    {
        MCU_LogShowHalState(halState,SPI2_DMA_RX_Handler.ErrorCode);
        while(1);
    }
    //接收DMA中断设定
...
ERROR_SUB MCU_SPI2_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    {
        UserMemSet(bufferWithSpi1Send,0,LENGTH_BYTES_SPI2_DMA_BUF);
    }
    //获取锁
    MCU_SPI2_GetLock();
    for(indexUtil = 0; indexUtil < sendPackCountMax;indexUtil++)
    {
        ...
        {
            //接收数据清空
            UserMemSet(bufferWithSpi1Recv,0,LENGTH_BYTES_SPI2_DMA_BUF);
        }
        //清空信号量标记
        ...
        //DMA数据发送,同时收发
        halState = HAL_SPI_TransmitReceive_DMA(&SPI2_Handler, (uint8_t*)bufferWithSpi1Send, (uint8_t *)bufferWithSpi1Recv, 
                                                    singlePackLength);
        if(halState != HAL_OK)
        {
            //释放锁
        ...
        if(readDataPtr != SPI_PTR_NULL)
        {
            ...
            //SCB_InvalidateDCache_by_Addr((uint32_t *)bufferWithSpi1Recv, LENGTH_BYTES_SPI2_DMA_BUF);
            //完成之后进行接收数据拷贝
