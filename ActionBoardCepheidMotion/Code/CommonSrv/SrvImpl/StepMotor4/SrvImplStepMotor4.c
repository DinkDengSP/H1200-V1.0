...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor4Cmd(void* canMsgBodyPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor4Upload(void* canMsgBodyPtr);
//CAN2指令消息处理函数
extern void SrvMsgCan2StepMotor4Cmd(void* canMsgBodyPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2StepMotor4Upload(void* canMsgBodyPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor4Cmd(void* ipcMsgBodyPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor4Upload(void* ipcMsgBodyPtr);
...
void SrvImplStepMotor4MsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_CMD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor4Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_UPLOAD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor4Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
