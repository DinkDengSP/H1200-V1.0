...
** LastEditTime  : 2023-12-18 09:57:49 +0800
************************************************************************************************/ 
...
/*---------------------------------------------------液面探测通讯配置----------------------------------------------------------*/
//COMM_LIQUID_DETECT 通讯波特率
#define BOARD_LIQUID_DETECT_BAUD                                38400
//COMM_LIQUID_DETECT 单字节长度                     
#define BOARD_LIQUID_DETECT_LENGTH                              MCU_UART_LENGTH_8B
//COMM_LIQUID_DETECT 停止位                     
#define BOARD_LIQUID_DETECT_STOP_BIT                            MCU_UART_STOPBIT1
//COMM_LIQUID_DETECT 校验模式                       
#define BOARD_LIQUID_DETECT_CHECK_MODE                          MCU_UART_CHECK_MODE_NONE
//COMM_LIQUID_DETECT 硬件流控制                     
#define BOARD_LIQUID_DETECT_HARD_CONTROL                        MCU_UART_HARD_CONTROL_NONE
//探液板接收消息缓冲区,最好和MCU串口接收数据块保持一致      
#define LENGTH_QUEUE_BOARD_LIQUID_DETECT_RECV_MSG               COUNT_UART1_RECV_BLOCK
//探液板消息处理循环        
#define PERIOD_MS_PROC_BOARD_LIQUID_DETECT_RECV_MSG             20
//通讯缓存数据长度      
#define COMM_CACHE_LENGTH_BOARD_LIQUID_DETECT                   50
//通讯异常的重试次数        
#define RETRY_MAX_COMM_BOARD_LIQUID_DETECT                      3
//常规指令超时时间      
#define TIME_OUT_MS_BOARD_LIQUID_DETECT                         200
//打开探液超时时间      
#define TIME_OUT_MS_BOARD_LIQUID_DETECT_ENABLE                  3000
//串口数据源        
#define COMM_PACK_SRC_BOARD_LIQUID_DETECT                       UART_PACK_SRC_1
//探液板使用的通讯端口初始化
#define COMM_INIT_BOARD_LIQUID_DETECT(baud,length,stopbit,checkMode,hardControl)        \
                                                                MCU_UART1_Init(baud,length,stopbit,checkMode,hardControl)
//探液板方法注册
#define COMM_CALLBACL_REGISTER_BOARD_LIQUID_DETECT(recvIntCallBack,beforeSendCallBack,afterSendCallBack)    \
                                                                MCU_UART1_ConfigCallBack(recvIntCallBack,beforeSendCallBack,afterSendCallBack)
//处理完成的数据返回给底层驱动
#define RELEASE_PACK_BOARD_LIQUID_DETECT(packPtr)               MCU_UART1_RecvDatRelease(packPtr)
//发送指令
#define COMM_SEND_CMD_BUF_BOARD_LIQUID_DETECT(buf,len)          MCU_UART1_SendBuffer(buf,len)
