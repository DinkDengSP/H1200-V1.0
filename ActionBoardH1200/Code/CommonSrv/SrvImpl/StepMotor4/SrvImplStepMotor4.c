...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor4Cmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor4Upload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor4Cmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor4Upload(void* ipcReportDataPtr);
//串口指令消息处理函数
extern void SrvMsgUartStepMotor4Cmd(void* uartCmdRecvPtr);
//串口上传消息处理函数
extern void SrvMsgUartStepMotor4Upload(void* uartReportDataPtr);
...
void SrvImplStepMotor4MsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            SrvMsgCan1StepMotor4Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            SrvMsgCan1StepMotor4Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            SrvMsgIpcStepMotor4Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            SrvMsgIpcStepMotor4Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_CMD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartStepMotor4Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_UPLOAD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartStepMotor4Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
