...
//CAN1指令消息处理函数
extern void ModMsgCan1NeedleReagentCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void ModMsgCan1NeedleReagentUpload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void ModMsgIpcNeedleReagentCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void ModMsgIpcNeedleReagentUpload(void* ipcReportDataPtr);
...
void ModImplNeedleReagentMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleReagentCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleReagentUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleReagentCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleReagentUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
