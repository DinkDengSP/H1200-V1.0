...
** Date          : 2023-11-07 09:10:06 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2023-11-07 09:10:06 +0800
************************************************************************************************/ 
#include "SysParam.h"
...
static void SysParamSetDefault(void)
{
    ...
    BoardParamSetNetPhy1DefaultIp();
    BoardParamSetNetPhy2DefaultIp();
    //将设定好的值写回外置Flash
    BOARD_PARAM_WRITE(ADDR_BOARD_SRV_PARAM_START,(uint8_t*)sysParamPtr,LENGTH_SYS_PARAM);
}

//通用的写入Flash的函数
void SysParamWriteSingleCommon(int32_t* targetParamPtr,int32_t writeParam)
{
...
/*----------------------------------------------------------------------------------------------------*/
//读写网络IP
ERROR_SUB SysParamReadParamNetIp(uint8_t subIndex,int32_t* readParam)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    switch(subIndex)
    {
        case INDEX_SUB_PARAM_NET_IP_PHY1:
            errorSub = BoardParamReadNetPhy1IpCurrent((uint32_t*)readParam);
            if(errorSub != ERROR_SUB_OK)
            {
                *readParam = 0;
                return errorSub;
...
ERROR_SUB SysParamWriteParamNetIp(uint8_t subIndex,int32_t writeParam)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    uint32_t writeVal = *((uint32_t*)(&writeParam));
    switch(subIndex)
    {
        case INDEX_SUB_PARAM_NET_IP_PHY1:
            errorSub = BoardParamSetNetPhy1Ip(writeVal);
            if(errorSub != ERROR_SUB_OK)
            {
                return errorSub;
            }
