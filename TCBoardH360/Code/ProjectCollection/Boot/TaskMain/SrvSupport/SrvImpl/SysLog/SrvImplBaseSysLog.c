...
//系统日志打印的互斥信号量
static TX_MUTEX mutexSrvTaskSysLog;
//用于printf的缓冲区
static uint8_t blockBufferSystemLog[BLOCK_SIZE_SYSTEM_LOG] = {0};
//获取锁
static void SysLogGetLock(void)
{
    tx_mutex_get(&mutexSrvTaskSysLog,TX_WAIT_FOREVER);
}
//释放锁
static void SysLogReleaseLock(void)
{
...
void SrvImplBaseSysLogSendBuffer(uint8_t* bufferPtr,uint16_t bufferLength)
{
    ...
    MCU_UART1_SendBuffer(bufferPtr,bufferLength);
#endif 
    //CAN1 SOCKET输出
#if(SYSTEM_LOG_OUT_SOCKET_CAN1_ENABLE == CONFIG_FUNC_ENABLE)
    if(SET == Can1ProtInitQuery())
    {
        Can1ProtSendLogSample(bufferPtr,bufferLength);
    }
#endif 
}
