...
void SrvMsgIpcDemoCmd(void* ipcMsgBodyPtr)
{
    ...
    for(indexDemo = 0; indexDemo < DIM_ARRAY_ELEMENT_COUNT(ipcDemoMsgCmdProcUnitArray);indexDemo++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcDemoMsgCmdProcUnitArray[indexDemo].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexDemo == DIM_ARRAY_ELEMENT_COUNT(ipcDemoMsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcDemoMsgCmdProcUnitArray[indexDemo].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcDemoMsgCmdProcUnitArray[indexDemo].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcDemoMsgCmdProcUnitArray[indexDemo].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcDemoMsgCmdProcUnitArray[indexDemo].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
void SrvMsgIpcDemoUpload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexDemo = 0;
    for(indexDemo = 0; indexDemo < DIM_ARRAY_ELEMENT_COUNT(ipcDemoMsgUploadProcUnitArray);indexDemo++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcDemoMsgUploadProcUnitArray[indexDemo].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexDemo == DIM_ARRAY_ELEMENT_COUNT(ipcDemoMsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcDemoMsgUploadProcUnitArray[indexDemo].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcDemoMsgUploadProcUnitArray[indexDemo].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcDemoMsgUploadProcUnitArray[indexDemo].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcDemoMsgUploadProcUnitArray[indexDemo].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
