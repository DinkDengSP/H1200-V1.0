...
void SrvMsgIpcPortInCmd(void* ipcMsgBodyPtr)
{
    ...
    for(indexPortIn = 0; indexPortIn < DIM_ARRAY_ELEMENT_COUNT(ipcPortInMsgCmdProcUnitArray);indexPortIn++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcPortInMsgCmdProcUnitArray[indexPortIn].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexPortIn == DIM_ARRAY_ELEMENT_COUNT(ipcPortInMsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcPortInMsgCmdProcUnitArray[indexPortIn].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcPortInMsgCmdProcUnitArray[indexPortIn].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcPortInMsgCmdProcUnitArray[indexPortIn].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcPortInMsgCmdProcUnitArray[indexPortIn].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
void SrvMsgIpcPortInUpload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexPortIn = 0;
    for(indexPortIn = 0; indexPortIn < DIM_ARRAY_ELEMENT_COUNT(ipcPortInMsgUploadProcUnitArray);indexPortIn++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcPortInMsgUploadProcUnitArray[indexPortIn].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexPortIn == DIM_ARRAY_ELEMENT_COUNT(ipcPortInMsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcPortInMsgUploadProcUnitArray[indexPortIn].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcPortInMsgUploadProcUnitArray[indexPortIn].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcPortInMsgUploadProcUnitArray[indexPortIn].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcPortInMsgUploadProcUnitArray[indexPortIn].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
