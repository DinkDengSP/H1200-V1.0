...
/*******************************************CAN指令处理函数列表*************************************************/
//占位函数
static void SrvMsgCan1TempSampleCmd_PlaceHolder(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);

//CAN指令处理函数列表
static const USER_CAN_CMD_PROC_UNIT can1CmdProcTempSampleUnitArray[] = 
{
    //顺序时 指令码 预期参数最小长度 是否需要结果返回  指令处理函数
    {CAN_MASTER_CMD_PLACE_HOLDER     ,1  ,ENABLE ,SrvMsgCan1TempSampleCmd_PlaceHolder  },
};

//CAN指令消息处理函数
void SrvMsgCan1TempSampleCmd(void* canMsgBodyPtr)
{
    //转换指令码
    ...
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(can1CmdProcTempSampleUnitArray);indexUtil++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can1CmdProcTempSampleUnitArray[indexUtil].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(can1CmdProcTempSampleUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can1CmdProcTempSampleUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can1CmdProcTempSampleUnitArray[indexUtil].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can1CmdProcTempSampleUnitArray[indexUtil].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can1CmdProcTempSampleUnitArray[indexUtil].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can1CmdProcTempSampleUnitArray[indexUtil].needResultReply == ENABLE)
    {
        //需要结果回传
...
/*************************************************************CAN上传参数处理***********************************************************/
//占位函数
static void SrvMsgCan1TempSampleUpload_PlaceHolder(CAN_PROT_UPLOAD_PACK* canUploadPackRecvPtr);

//CAN上传消息处理函数列表
static const USER_CAN_UPLOAD_PROC_UNIT canMsgUploadProcessUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {CAN_MASTER_REPORT_ID_HOLD_PLACE     ,2 ,SrvMsgCan1TempSampleUpload_PlaceHolder},
};

//CAN上传消息处理函数
void SrvMsgCan1TempSampleUpload(void* canMsgBodyPtr)
{
