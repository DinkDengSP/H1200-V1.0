...
void SrvMsgIpcStepMotor7Cmd(void* ipcMsgBodyPtr)
{
    ...
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor7MsgCmdProcUnitArray);indexUtil++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcStepMotor7MsgCmdProcUnitArray[indexUtil].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor7MsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcStepMotor7MsgCmdProcUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcStepMotor7MsgCmdProcUnitArray[indexUtil].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcStepMotor7MsgCmdProcUnitArray[indexUtil].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcStepMotor7MsgCmdProcUnitArray[indexUtil].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
static void SrvMsgIpcStepMotor7Cmd_ReturnZero(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr)
{
    ...
    channelCachePtr->returnDataBufferLength = 0;
    //调用API
    SrvImplBaseStepMotor7ReturnZero(&returnZeroCmd,needReset,resultPackPtr,&returnBufferPtr,&returnBufferLength);
    //对结果进行处理
    if(resultPackPtr->resultCode == PROT_RESULT_SUCCESS)
    {
        if((returnBufferPtr != NULL)&&(returnBufferLength != 0))
...
void SrvMsgIpcStepMotor7Upload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexUtil = 0;
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor7MsgUploadProcUnitArray);indexUtil++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcStepMotor7MsgUploadProcUnitArray[indexUtil].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(ipcStepMotor7MsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcStepMotor7MsgUploadProcUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcStepMotor7MsgUploadProcUnitArray[indexUtil].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcStepMotor7MsgUploadProcUnitArray[indexUtil].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcStepMotor7MsgUploadProcUnitArray[indexUtil].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
