 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-02-02 13:42:59
...
/*************************************************************IPC指令处理处理***********************************************************/
//通讯测试
static void SrvMsgIpcTimerCmd_CommTest(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);

//IPC指令处理函数列表
static const USER_IPC_CMD_PROC_UNIT ipcTimerMsgCmdProcUnitArray[] = 
{
    //顺序是 指令码 预期参数最小长度  指令处理函数
    {(uint32_t)0XFFFFFFFF     ,0    ,SrvMsgIpcTimerCmd_CommTest  },
};


//IPC指令消息处理函数
void SrvMsgIpcTimerCmd(void* ipcMsgBodyPtr)
{
    //转换指令码
    ...
    for(indexTimer = 0; indexTimer < DIM_ARRAY_ELEMENT_COUNT(ipcTimerMsgCmdProcUnitArray);indexTimer++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcTimerMsgCmdProcUnitArray[indexTimer].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexTimer == DIM_ARRAY_ELEMENT_COUNT(ipcTimerMsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcTimerMsgCmdProcUnitArray[indexTimer].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcTimerMsgCmdProcUnitArray[indexTimer].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcTimerMsgCmdProcUnitArray[indexTimer].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcTimerMsgCmdProcUnitArray[indexTimer].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
/*************************************************************IPC上报讯息处理**********************************************************/
//占位函数
static void SrvMsgIpcTimerUpload_PlaceHolder(IPC_UPLOAD_DATA* ipcUploadDataPtr);

//IPC上传消息处理函数列表
static const USER_IPC_UPLOAD_PROC_UNIT ipcTimerMsgUploadProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)0XFFFFFFFF     ,2 ,SrvMsgIpcTimerUpload_PlaceHolder},
};


//IPC上传消息处理函数
void SrvMsgIpcTimerUpload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexTimer = 0;
    for(indexTimer = 0; indexTimer < DIM_ARRAY_ELEMENT_COUNT(ipcTimerMsgUploadProcUnitArray);indexTimer++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcTimerMsgUploadProcUnitArray[indexTimer].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexTimer == DIM_ARRAY_ELEMENT_COUNT(ipcTimerMsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcTimerMsgUploadProcUnitArray[indexTimer].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcTimerMsgUploadProcUnitArray[indexTimer].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcTimerMsgUploadProcUnitArray[indexTimer].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcTimerMsgUploadProcUnitArray[indexTimer].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
