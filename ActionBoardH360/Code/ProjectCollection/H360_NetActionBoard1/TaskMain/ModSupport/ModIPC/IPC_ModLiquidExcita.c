...
** LastEditTime  : 2023-11-26 23:20:30 +0800
************************************************************************************************/ 
#include "IPC_ModLiquidExcita.h"
#include "ModTaskHeader.h"
#include "IPC_Port.h"

//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_MOD_LIQUID_EXCITA             128
//IPC通道讯息
static IPC_CHANNEL_DATA ipcChannelDataModLiquidExcita;
//通道名称
static const uint8_t* ipcChannelModLiquidExcitaNameStr = "ModLiquidExcita";

/*****************************************************通道控制************************************************************/
//初始化IPC通道
void IPC_ModLiquidExcitaInit(void)
{
    IPC_BaseInitChannelData(&ipcChannelDataModLiquidExcita,(uint8_t*)ipcChannelModLiquidExcitaNameStr,
                                &queueModTaskUtil,LENGTH_BUFFER_IPC_MOD_LIQUID_EXCITA);
}

//清空IPC信道
ERROR_SUB IPC_ModLiquidExcitaClearChannelByCmd(uint32_t cmdCode)
{
    return IPC_BaseChannelClearChannel(&ipcChannelDataModLiquidExcita,cmdCode);
}
...
ERROR_SUB IPC_ModLiquidExcitaClearChannel(void)
{
    ...
    //首先清除通讯测试指令
    errorSub = IPC_BaseChannelClearChannel(&ipcChannelDataModLiquidExcita,IPC_CMD_MACRO_COMM_TEST);
    ...
    {
        errorSub = IPC_BaseChannelClearChannel(&ipcChannelDataModLiquidExcita,cmdCode);
        if(errorSub != ERROR_SUB_OK)
        {
...
void IPC_ModLiquidExcitaWaitCmdReturn(uint32_t cmdCode,SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    //等待指令返回
    errorSub = IPC_BaseWaitCmdReturn(&ipcChannelDataModLiquidExcita,cmdCode,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,Cmd: 0X%08X,ErrorSub: 0X%08X\r\n",__func__,cmdCode,errorSub);
        //IPC报错,返回错误
    ...
    if((ipcChannelDataModLiquidExcita.channelCache.returnDataBufferLength != 0)&&(returnDataPtr != NULL))
    {
        //申请内存
        returnDataPtr->returnDataLength = ipcChannelDataModLiquidExcita.channelCache.returnDataBufferLength;
        //不为0拷贝数据
        if(returnDataPtr->returnDataLength != 0)
        {
            ...
            //数据拷贝
            UserMemCopy(returnDataPtr->returnBufferPtr,ipcChannelDataModLiquidExcita.channelCache.dataBufferPtr,returnDataPtr->returnDataLength);
        }
    }
    return;
}
...
void IPC_ModLiquidExcitaWaitLastCmdReturn(SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    //等待指令返回
    errorSub = IPC_BaseWaitLastCmdReturn(&ipcChannelDataModLiquidExcita,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
                                        ...
                                        IPC_RETURN_DATA* returnDataPtr)
{
    ...
    errorSub = IPC_BaseQueryCmdState(&ipcChannelDataModLiquidExcita,cmdCode,cmdStagePtr,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,Cmd: 0X%08X,ErrorSub: 0X%08X\r\n",__func__,cmdCode,errorSub);
        //IPC报错,返回错误
...
void IPC_ModLiquidExcitaQueryLastCmdState(IPC_CMD_STAGE* cmdStagePtr,SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    errorSub = IPC_BaseQueryLastCmdState(&ipcChannelDataModLiquidExcita,cmdStagePtr,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,ErrorSub: 0X%08X\r\n",__func__,errorSub);
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_IPC_PROT_QUERY_CMD_STATE,ERROR_LEVEL_ERROR,
...
/*****************************************************发送指令等待ACK************************************************************/
static ERROR_SUB IPC_ModLiquidExcitaCheckClearLastCmd(uint32_t cmdCode)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    PROT_CH_STA channelState;
    //首先检查是否上个数据未完成或者完成之后未读取
    channelState = IPC_BaseCheckCmdChannelState(&ipcChannelDataModLiquidExcita,cmdCode);
...
void IPC_ModLiquidExcitaCommTestWhileAck(SYS_RESULT_PACK* resultPackPtr)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //清除上一个指令
    errorSub = IPC_ModLiquidExcitaCheckClearLastCmd(IPC_CMD_MACRO_COMM_TEST);
    if(errorSub != ERROR_SUB_OK)
    {
    ...
    errorSub = IPC_BaseSendCmdWhileAck(&ipcChannelDataModLiquidExcita,&ipcCmdBaseData);
    if(errorSub != ERROR_SUB_OK)
    {
...
void IPC_ModLiquidExcitaCommTestWhileReturn(SYS_RESULT_PACK* resultPackPtr,IPC_RETURN_DATA* returnDataPtr)
{
    ...
    ipcCmdBaseData.commandCode = IPC_CMD_MACRO_COMM_TEST;
    //清除掉上一次的指令了,进行下一次指令发送
    errorSub = IPC_BaseSendCmdWhileReturn(&ipcChannelDataModLiquidExcita,&ipcCmdBaseData,resultPackPtr);
    if(errorSub != ERROR_SUB_OK)
    {
        SystemPrintf("%s,Cmd: 0X%08X,ErrorSub: 0X%08X\r\n",__func__,ipcCmdBaseData.commandCode,
                        errorSub);
