...
** LastEditTime  : 2024-01-03 10:06:03 +0800
************************************************************************************************/ 
...
//BOOT升级,回传一个升级准备OK标记
void SrvImplFlashIapBootUpdata(BOARD_IAP_RECODER* iapRecordPtr)
{
    ...
    if(iapRecordPtr->boardIapSrc == BOARD_IAP_SRC_CAN1)
    {
        //清除成功,回传升级准备成功标记
    ...
    else if(iapRecordPtr->boardIapSrc == BOARD_IAP_SRC_CAN2)
    {
        //清除成功,回传升级准备成功标记
        Can2ProtSendResult(SYSTEM_MODULE_NO_PC,SYSTEM_BOARD_ID_PC,CAN_MASTER_CH_ACT_IAP,
                            CAN_MASTER_CMD_IAP_PREPARE,resultPack.resultCode,resultPack.errorMain,
                            resultPack.errorLevel,resultPack.errorSub, NULL,0,
                            &resultPack);
        if(resultPack.resultCode != PROT_RESULT_SUCCESS)
        {
            SystemPrintf("E:> %s,%d,Can2ProtSendResult Failed,ErrorSub: 0X%08X\r\n",__func__,__LINE__,
                            resultPack.errorSub);
            return;
        }
    }
...
void SrvImplBaseFlashIapStartMsg(BOARD_IAP_FILE_INFO* iapInfoPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    errorSub = MCU_FlashEraseRangeWithCheck(MCU_FLASH_BOOT,eraseWord);
    if(errorSub != ERROR_SUB_OK)
    {
                                    ...
                                    uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //flash写入之后的比对
    MCU_FLASH_CMP_RESULT flashCompare = MCU_FLASH_CMP_EQU;
    //重试记录
    ...
    do
    {
        retryCount++;
        //写入对应的Flash,APP
        errorSub = MCU_FlashWrite(MCU_FLASH_BOOT+boardIapFileInfo.recordRecvFileByteCount,(uint32_t*)writeBufferPtrAlign4Byte,
                                    packDataLengthAlign4Byte/4);
        if(errorSub != ERROR_SUB_OK)
        {
            //写入失败
        ...
        //写入完成,进行比对
        flashCompare = MCU_FlashCompare(MCU_FLASH_BOOT+boardIapFileInfo.recordRecvFileByteCount,(uint8_t*)writeBufferPtrAlign4Byte,
                                            packDataLengthAlign4Byte);
        if(flashCompare != MCU_FLASH_CMP_EQU)
        {
            //比对数据失败
            continue;
        }
    ...
    {
        //错误日志
        SystemPrintf("%s,%d,MCU_FlashCompare Failed,flashCompare: %d\r\n",__func__,__LINE__,flashCompare);
        BoardIapClearFileInfo(&boardIapFileInfo);
        //设置错误
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_ACT_FLASH_IAP_DATA,
                                    ERROR_LEVEL_ERROR,errorSub);
