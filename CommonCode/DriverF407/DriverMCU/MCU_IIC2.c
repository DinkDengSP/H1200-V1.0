...
** Date          : 2023-10-30 11:46:02 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2023-10-30 11:46:14 +0800
************************************************************************************************/ 
#include "MCU_IIC2.h"
#include "MCU_Pin.h"
...
static void MCU_IIC2_SDA_DirSet(SDA_DIR dir)
{
    ...
    {
        //输出
        MCU_PinInit(PIN_MCU_IIC2_SDA, MCU_PIN_MODE_OUTPUT_OD, MCU_PIN_PULL_NOPULL, MCU_PIN_SPEED_FREQ_MEDIUM,0);
    }
    else
    {
        //输入
        MCU_PinInit(PIN_MCU_IIC2_SDA, MCU_PIN_MODE_INPUT, MCU_PIN_PULL_NOPULL, MCU_PIN_SPEED_FREQ_MEDIUM,0);
    }
}
...
void MCU_IIC2_PortInit(void)
{
    ...
    //同时还能赋予IIC设备的线与能力
    MCU_PinInit(PIN_MCU_IIC2_SCL, MCU_PIN_MODE_OUTPUT_OD, MCU_PIN_PULL_NOPULL, MCU_PIN_SPEED_FREQ_MEDIUM,0);
    MCU_PinInit(PIN_MCU_IIC2_SDA, MCU_PIN_MODE_OUTPUT_OD, MCU_PIN_PULL_NOPULL, MCU_PIN_SPEED_FREQ_MEDIUM,0);
    //设置SDA处于输出模式
    sdaDir = SDA_DIR_OUT;
...
HAL_StatusTypeDef MCU_IIC2_CheckDeviceReady(uint8_t deviceAddr)
{
    ...
    if(needWaitTimeTick < MS_TO_OS_TICK(IIC2_TIME_OUT_MS))
    {
        needWaitTimeTick = IIC2_TIME_OUT_MS - OS_TICK_TO_MS(needWaitTimeTick);
...
void MCU_IIC2_PortInit(void)
{
    ...
    //首先端口初始化,IIC端口在链接多个设备的时候,需要外部上拉电阻,单片机引脚设置为开漏,这样能控制IIC速率
    //同时还能赋予IIC设备的线与能力
    MCU_PinInit(PIN_MCU_IIC2_SCL, MCU_PIN_MODE_AF_OD, MCU_PIN_PULL_NOPULL, MCU_PIN_SPEED_FREQ_MEDIUM,GPIO_AF4_I2C2);
    MCU_PinInit(PIN_MCU_IIC2_SDA, MCU_PIN_MODE_AF_OD, MCU_PIN_PULL_NOPULL, MCU_PIN_SPEED_FREQ_MEDIUM,GPIO_AF4_I2C2);
    //IIC外设初始化
    IIC2_Handle.Instance = I2C2;
    //IIC复位
    HAL_I2C_DeInit(&IIC2_Handle);
    //IIC核心时序
    IIC2_Handle.Init.ClockSpeed       = MCU_IIC2_CLK_SPEED;
    IIC2_Handle.Init.DutyCycle        = MCU_IIC2_DUTY_CYCLE;
...
HAL_StatusTypeDef MCU_IIC2_CheckDeviceReady(uint8_t deviceAddr)
{
    ...
    //关闭中断
    __HAL_I2C_DISABLE_IT(&IIC2_Handle,I2C_IT_BUF|I2C_IT_ERR);
    //检查是否准备好
    halState = HAL_I2C_IsDeviceReady(&IIC2_Handle, deviceAddr, 1, IIC2_TIME_OUT_MS);
    //已经执行的时间
    needWaitTimeTick = tx_time_get() - timeTickStart;
    if((halState != HAL_OK)&&(needWaitTimeTick < MS_TO_OS_TICK(IIC2_TIME_OUT_MS)))
    {
        needWaitTimeTick = IIC2_TIME_OUT_MS - OS_TICK_TO_MS(needWaitTimeTick);
        CoreDelayMs(needWaitTimeTick);
    }
    //清除全部中断
    __HAL_I2C_CLEAR_FLAG(&IIC2_Handle,I2C_FLAG_OVR|I2C_FLAG_AF|I2C_FLAG_ARLO|I2C_FLAG_BERR|
        I2C_FLAG_TXE|I2C_FLAG_RXNE|I2C_FLAG_STOPF|I2C_FLAG_ADD10|I2C_FLAG_BTF|I2C_FLAG_ADDR|
        I2C_FLAG_SB|I2C_FLAG_DUALF|I2C_FLAG_GENCALL|I2C_FLAG_TRA|I2C_FLAG_BUSY|I2C_FLAG_MSL);
    //释放信号量
    MCU_IIC2_ReleaseLock();
#if(TIME_MEASURE_MCU_IIC2 == CONFIG_FUNC_ENABLE)
    MCU_LogPrintf("%s,DeviceAddr: 0X%02X,timeOutMax: %d,TimeUse: %d MS\r\n",__func__,deviceAddr,IIC2_TIME_OUT_MS,
                                                    ...
                                                    uint8_t* dataBuffer,uint8_t dataLength,uint8_t singleByteTimeOut)
{
    ...
    if(halState != HAL_OK)
    {
        //释放信号量
        MCU_IIC2_ReleaseLock();
        MCU_LogShowHalState(halState,IIC2_Handle.ErrorCode);
        return halState;
    }
    //释放信号量
    MCU_IIC2_ReleaseLock();
...
//错误事件
#define IIC2_EVENT_FLAG_ERR_OCCOR        (0X00000001)
//终止事件
#define IIC2_EVENT_FLAG_ABORT            (0X00000002)
//数据发送事件
#define IIC2_EVENT_FLAG_DATA_WRITE       (0X00000004)
//数据接收事件
#define IIC2_EVENT_FLAG_DATA_READ        (0X00000008)
//寄存器写入事件
#define IIC2_EVENT_FLAG_MEM_WRITE        (0X00000010)
//寄存器读取事件
#define IIC2_EVENT_FLAG_MEM_READ         (0X00000020)


//错误中断侦测
void MCU_IIC2_ErrorCallbackInt(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_ErrorCallbackInt\r\n");
#endif
}

//取消操作完成
void MCU_IIC2_AbortCpltCallbackInt(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ABORT,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_AbortCpltCallbackInt\r\n");
#endif
}

//发送完成回调
void MCU_IIC2_MemTxCpltCallbackInt(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_MEM_WRITE,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MemTxCpltCallbackInt\r\n");
#endif
}

//接收完成回调
void MCU_IIC2_MemRxCpltCallbackInt(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_MEM_READ,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MemRxCpltCallbackInt\r\n");
#endif
}

//发送完成回调
void MCU_IIC2_MasterTxCpltCallbackInt(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_DATA_WRITE,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MasterTxCpltCallbackInt\r\n");
#endif
}

//接收完成回调
void MCU_IIC2_MasterRxCpltCallbackInt(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_DATA_READ,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MasterRxCpltCallbackInt\r\n");
#endif
}
...
void MCU_IIC2_PortInit(void)
{
    ...
    //中断关联
    IIC2_Handle.ErrorCallback = MCU_IIC2_ErrorCallbackInt;
    IIC2_Handle.AbortCpltCallback = MCU_IIC2_AbortCpltCallbackInt;
    IIC2_Handle.MemTxCpltCallback = MCU_IIC2_MemTxCpltCallbackInt;
    IIC2_Handle.MemRxCpltCallback = MCU_IIC2_MemRxCpltCallbackInt;
    IIC2_Handle.MasterTxCpltCallback = MCU_IIC2_MasterTxCpltCallbackInt;
    IIC2_Handle.MasterRxCpltCallback = MCU_IIC2_MasterRxCpltCallbackInt;
    //配置IIC中断
    HAL_NVIC_SetPriority(I2C2_EV_IRQn, IIC2_EVENT_PREE_PRI, IIC2_EVENT_SUB_PRI);
    HAL_NVIC_EnableIRQ(I2C2_EV_IRQn);
    HAL_NVIC_SetPriority(I2C2_ER_IRQn, IIC2_ERR_PREE_PRI, IIC2_ERR_SUB_PRI);
...
static void MCU_IIC2_ClearAbortTransFlag(void)
{
    ...
    //首先清空标记
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ABORT,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
}
...
static HAL_StatusTypeDef MCU_IIC2_WaitAbortTransFlag(uint32_t timeOutMs)
{
    ...
    txResult = tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR|IIC2_EVENT_FLAG_ABORT,TX_OR_CLEAR,(ULONG*)&eventFlagsActualFlag,
                                            MS_TO_OS_TICK(timeOutMs));
    if(txResult == TX_SUCCESS)
    {
        if(eventFlagsActualFlag&IIC2_EVENT_FLAG_ERR_OCCOR)
        {
            MCU_LogShowHalState(HAL_ERROR,0);
            return HAL_ERROR;
        }
        return HAL_OK;
    }
                                                    ...
                                                    uint8_t* dataBuffer,uint8_t dataLength,uint8_t singleByteTimeOut)
{
    ...
    MCU_IIC2_GetLock();
    //首先清除发送信号量
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_MEM_WRITE,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    //数据写入
    if(subAddrSize == IIC_SUB_ADDR_SIZE_8BIT)
    {
    ...
    txResult = tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR|IIC2_EVENT_FLAG_MEM_WRITE,TX_OR_CLEAR,(ULONG*)&eventFlagActual,
    ...
    if(eventFlagActual&IIC2_EVENT_FLAG_ERR_OCCOR)
    {
        //超时,传输终止
        MCU_IIC2_AbortTrans(deviceAddr,IIC2_TIME_OUT_MS);
        //复位IIC
...
#if(TIME_MEASURE_MCU_IIC2 == CONFIG_FUNC_ENABLE)
    MCU_LogPrintf("%s,DeviceAddr: 0X%02X,memAddr: %d,dataLength: %d,TimeUse: %d MS\r\n",__func__,deviceAddr,memAddr,dataLength,
                        MS_TO_OS_TICK(tx_time_get()-timeTickStart));
#endif
    return HAL_OK;
}
                                                    ...
                                                    uint8_t* dataBuffer,uint8_t dataLength,uint8_t singleByteTimeOut)
{
    ...
    MCU_IIC2_GetLock();
    //首先清除接收信号量
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_MEM_READ,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    //数据读取
    if(subAddrSize == IIC_SUB_ADDR_SIZE_8BIT)
    {
...
#endif
    uint32_t txResult = TX_SUCCESS;
    txResult = tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR|IIC2_EVENT_FLAG_MEM_READ,TX_OR_CLEAR,(ULONG*)&eventFlagActual,
                                            MS_TO_OS_TICK(singleByteTimeOut*dataLength));
    if(txResult != TX_SUCCESS)
    {
        //超时,传输终止
                                            ...
                                            uint8_t singleByteTimeOut)
{
    ...
    MCU_IIC2_GetLock();
    //清除发送信号量
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_DATA_WRITE,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    //数据发送
    ...
    //等待发送完成
    uint32_t txResult = TX_SUCCESS;
    txResult = tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR|IIC2_EVENT_FLAG_DATA_WRITE,TX_OR_CLEAR,(ULONG*)&eventFlagActual,
                                            MS_TO_OS_TICK(singleByteTimeOut*dataLength));
    if(txResult != TX_SUCCESS)
    {
        //超时,传输终止
                                            ...
                                            uint8_t singleByteTimeOut)
{
    ...
    MCU_IIC2_GetLock();
    //首先清除接收信号量
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_DATA_READ,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    while(TX_SUCCESS == tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_AND_CLEAR,(ULONG*)&eventFlagActual,TX_NO_WAIT));
    ...
    //等待接收完成
    uint32_t txResult = TX_SUCCESS;
    txResult = tx_event_flags_get(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR|IIC2_EVENT_FLAG_DATA_READ,TX_OR_CLEAR,(ULONG*)&eventFlagActual,
                                            MS_TO_OS_TICK(singleByteTimeOut*dataLength));
    if(txResult != TX_SUCCESS)
    {
        //超时,传输终止
...
#pragma pack(4)
//用于DMA发送数据快
static uint8_t bufferTransWithIIC2[IIC2_BUFFER_LENGTH_DMA_MODE]@".sramin" = {0};
//用于DMA接收数据快
static uint8_t bufferRecvWithIIC2[IIC2_BUFFER_LENGTH_DMA_MODE]@".sramin" = {0};
#pragma pack (pop)
...
//错误中断侦测
void MCU_IIC2_ErrorCallbackDMA(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ERR_OCCOR,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_ErrorCallbackDMA\r\n");
#endif
}

//取消操作完成
void MCU_IIC2_AbortCpltCallbackDMA(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_ABORT,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_AbortCpltCallbackDMA\r\n");
#endif
}

//发送完成回调
void MCU_IIC2_MemTxCpltCallbackDMA(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_MEM_WRITE,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MemTxCpltCallbackDMA\r\n");
#endif
}

//接收完成回调
void MCU_IIC2_MemRxCpltCallbackDMA(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_MEM_READ,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MemRxCpltCallbackDMA\r\n");
#endif
}

//发送完成回调
void MCU_IIC2_MasterTxCpltCallbackDMA(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_DATA_WRITE,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MasterTxCpltCallbackDMA\r\n");
#endif
}

//接收完成回调
void MCU_IIC2_MasterRxCpltCallbackDMA(I2C_HandleTypeDef* handlePtrIIC)
{
    tx_event_flags_set(&flagGroupMCU_IIC2,IIC2_EVENT_FLAG_DATA_READ,TX_OR);
#if(MCU_IIC2_SHOW_INT_MSG == CONFIG_FUNC_ENABLE)
    MCU_LogSendStringInt("MCU_IIC2_MasterRxCpltCallbackDMA\r\n");
#endif
}
...
static void MCU_IIC2_PortDMA_Init(void)
{
    ...
    IIC2_DMA_TX_Handler.Init.Channel             = IIC2_DMA_TX_CHANNEL;
    ...
    //发送DMA中断设定
    HAL_NVIC_SetPriority(IIC2_DMA_TX_IRQ, IIC2_DMA_TX_PREE_PRI, IIC2_DMA_TX_SUB_PRI);
    ...
    IIC2_DMA_RX_Handler.Init.Channel             = IIC2_DMA_RX_CHANNEL;
    ...
    //接收DMA中断设定
    HAL_NVIC_SetPriority(IIC2_DMA_RX_IRQ, IIC2_DMA_RX_PREE_PRI, IIC2_DMA_RX_SUB_PRI);
...
void MCU_IIC2_PortInit(void)
{
    ...
    //中断关联
    IIC2_Handle.ErrorCallback = MCU_IIC2_ErrorCallbackDMA;
    IIC2_Handle.AbortCpltCallback = MCU_IIC2_AbortCpltCallbackDMA;
    IIC2_Handle.MemTxCpltCallback = MCU_IIC2_MemTxCpltCallbackDMA;
    IIC2_Handle.MemRxCpltCallback = MCU_IIC2_MemRxCpltCallbackDMA;
    IIC2_Handle.MasterTxCpltCallback = MCU_IIC2_MasterTxCpltCallbackDMA;
    IIC2_Handle.MasterRxCpltCallback = MCU_IIC2_MasterRxCpltCallbackDMA; 
    //配置IIC中断
    HAL_NVIC_SetPriority(I2C2_EV_IRQn, IIC2_EVENT_PREE_PRI, IIC2_EVENT_SUB_PRI);
    HAL_NVIC_EnableIRQ(I2C2_EV_IRQn);
    HAL_NVIC_SetPriority(I2C2_ER_IRQn, IIC2_ERR_PREE_PRI, IIC2_ERR_SUB_PRI);
