...
//CAN2指令消息处理函数
extern void SrvMsgCan2StepMotor2Cmd(void* canMsgBodyPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2StepMotor2Upload(void* canMsgBodyPtr);
//IPC指令消息处理函数
...
void SrvImplStepMotor2MsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_CMD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor2Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_UPLOAD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor2Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
