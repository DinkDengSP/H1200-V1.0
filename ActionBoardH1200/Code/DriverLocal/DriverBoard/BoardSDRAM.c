...
//IS42S32800G读写序列初始化
static void BoardIS42S16400SequenceInit(SDRAM_HandleTypeDef *hsdram, FMC_SDRAM_CommandTypeDef *Command)
{
...
void BoardSDRAM_Init(void)
{
	...
	//FMC使用的PLL2R时钟，240MHz，用于SDRAM的话，至少3分频，也就是80M，即1个SDRAM时钟周期是12.5ns,下面参数单位均为12.5ns。
    SDRAM_Timing.LoadToActiveDelay    = 2; /* 最小值12ns,实际12.5*2ns=25ns, TMRD定义加载模式寄存器的命令与激活命令或刷新命令之间的延迟 */
    SDRAM_Timing.ExitSelfRefreshDelay = 7; /* 最小值72ns,实际12.5*7ns=87.5ns, TXSR定义从发出自刷新命令到发出激活命令之间的延迟 */
    SDRAM_Timing.SelfRefreshTime      = 4; /* 最小值42ns,实际12.5*4ns=50ns, TRAS定义最短的自刷新周期 */
    SDRAM_Timing.RowCycleDelay        = 6; /* 最小值60ns,实际12.5*6ns=75ns, TRC定义刷新命令和激活命令之间的延迟 */
    SDRAM_Timing.WriteRecoveryTime    = 2; /* 最小值12ns,实际12.5*2ns=25ns, TWR定义在写命令和预充电命令之间的延迟 */
    SDRAM_Timing.RPDelay              = 2; /* 最小值18ns,实际12.5*2ns=25ns, TRP定义预充电命令与其它命令之间的延迟 */
    SDRAM_Timing.RCDDelay             = 2; /* 最小值18ns,12.5*2=25ns, TRCD定义激活命令与读/写命令之间的延迟 */
    //BANK2的设置
    ...
    hsdram.Init.SDClockPeriod      = FMC_SDRAM_CLOCK_PERIOD_3;          /* FMC时钟240MHz，3分频后给SDRAM，即80MHz */
	...
	/* 完成SDRAM序列初始化 */
	BoardIS42S16400SequenceInit(&hsdram, &command);
}
...
ERROR_SUB BoardSDRAM_Check(void)
{
    ...
    //全部写入0XAA
    writeValue = 0XAA;
    opAddr = BOARD_SDRAM_BASE_ADDR;
    for(indexUtil = 0; indexUtil < BOARD_SDRAM_CAPACITY;indexUtil++)
    {
