...
//CAN1指令消息处理函数
extern void ModMsgCan1NeedleSampleCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void ModMsgCan1NeedleSampleUpload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void ModMsgIpcNeedleSampleCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void ModMsgIpcNeedleSampleUpload(void* ipcReportDataPtr);
...
void ModImplNeedleSampleMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleSampleCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1NeedleSampleUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleSampleCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcNeedleSampleUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
