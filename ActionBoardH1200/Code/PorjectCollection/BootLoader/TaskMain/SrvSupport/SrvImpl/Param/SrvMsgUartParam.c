 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-01-16 18:12:21
...
//参数单个读取        
static void SrvMsgUartParamCmd_ParamReadSingle(UART_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* sysResultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen); 
//参数批量读取
static void SrvMsgUartParamCmd_ParamReadAny(UART_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* sysResultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen); 
//参数单个写入   
static void SrvMsgUartParamCmd_ParamWriteSingle(UART_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* sysResultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//参数批量写入
static void SrvMsgUartParamCmd_ParamWriteAny(UART_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* sysResultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);   
...
static const USER_UART_CMD_PROC_UNIT uartCmdProcParamUnitArray[] = 
{
    ...
    {(uint32_t)UART_MASTER_CMD_MAIN_PARAM_RW_READ_SINGLE    ,0  ,ENABLE ,SrvMsgUartParamCmd_ParamReadSingle      },
    {(uint32_t)UART_MASTER_CMD_MAIN_PARAM_RW_READ_ANY       ,0  ,ENABLE ,SrvMsgUartParamCmd_ParamReadAny         },
    {(uint32_t)UART_MASTER_CMD_MAIN_PARAM_RW_WRITE_SINGLE   ,0  ,ENABLE ,SrvMsgUartParamCmd_ParamWriteSingle     },
    {(uint32_t)UART_MASTER_CMD_MAIN_PARAM_RW_WRITE_ANY      ,0  ,ENABLE ,SrvMsgUartParamCmd_ParamWriteAny        },
};
...
void SrvMsgUartParamCmd(void* uartCmdRecvPtr)
{
    ...
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(uartCmdProcParamUnitArray);indexUtil++)
    {
        if(cmdPackPtr->cmdCode == (uint32_t)(uartCmdProcParamUnitArray[indexUtil].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(uartCmdProcParamUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->dataBodyLength < uartCmdProcParamUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        uartCmdProcParamUnitArray[indexUtil].exceptParamLengthMin,cmdPackPtr->dataBodyLength,
                        cmdPackPtr->cmdCode);
        //释放内存
        if((cmdPackPtr->dataBodyBufferPtr != NULL)&&(cmdPackPtr->dataBodyLength != 0))
        {
    ...
    //检测函数指针
    if(uartCmdProcParamUnitArray[indexUtil].uartCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    uartCmdProcParamUnitArray[indexUtil].uartCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(uartCmdProcParamUnitArray[indexUtil].needResultReply == ENABLE)
    {
        //需要结果回传
...
void SrvMsgUartParamUpload(void* uartReportDataPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexUtil = 0;
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(uartUploadParamProcUnitArray);indexUtil++)
    {
        if(uartUploadPackRecvPtr->uploadID == (uint32_t)(uartUploadParamProcUnitArray[indexUtil].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(uartUploadParamProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(uartUploadPackRecvPtr->paramBufferLength < uartUploadParamProcUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        uartUploadParamProcUnitArray[indexUtil].exceptParamLengthMin,uartUploadPackRecvPtr->paramBufferLength,
    ...
    //检测函数指针
    if(uartUploadParamProcUnitArray[indexUtil].uartUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    uartUploadParamProcUnitArray[indexUtil].uartUploadProcFuncPtr(uartUploadPackRecvPtr);
    //处理完成,释放内存
    if((uartUploadPackRecvPtr->paramBufferPtr != NULL)&&(uartUploadPackRecvPtr->paramBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_UART,uartUploadPackRecvPtr->paramBufferPtr);
