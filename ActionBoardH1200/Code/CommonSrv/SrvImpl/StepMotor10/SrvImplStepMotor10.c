...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor10Cmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor10Upload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor10Cmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor10Upload(void* ipcReportDataPtr);
//串口指令消息处理函数
extern void SrvMsgUartStepMotor10Cmd(void* uartCmdRecvPtr);
//串口上传消息处理函数
extern void SrvMsgUartStepMotor10Upload(void* uartReportDataPtr);
...
void SrvImplStepMotor10MsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_CMD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartStepMotor10Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_UPLOAD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartStepMotor10Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
