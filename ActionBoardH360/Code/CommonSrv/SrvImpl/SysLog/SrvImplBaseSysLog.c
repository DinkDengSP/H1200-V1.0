...
** LastEditTime  : 2023-11-10 16:15:15 +0800
************************************************************************************************/ 
...
//输出
__weak void SrvImplBaseSysLogSendBuffer(uint8_t* logDataBuffer,uint16_t logDataLength)
{
    //RTT日志输出
#if(SYSTEM_LOG_OUT_RTT_ENABLE == CONFIG_FUNC_ENABLE)
    MCU_RTT_SendBuffer(logDataBuffer,logDataLength);
#endif
    //UART1日志输出
#if(SYSTEM_LOG_OUT_UART1_ENABLE == CONFIG_FUNC_ENABLE)
    MCU_UART1_SendBuffer(logDataBuffer,logDataLength);
#endif 
    //UART3日志输出
#if(SYSTEM_LOG_OUT_UART3_ENABLE == CONFIG_FUNC_ENABLE)
    MCU_UART3_SendBuffer(logDataBuffer,logDataLength);
#endif 
    //CAN1 SOCKET输出
#if(SYSTEM_LOG_OUT_SOCKET_CAN1_ENABLE == CONFIG_FUNC_ENABLE)
    if(SET == Can1ProtInitQuery())
    {
        Can1ProtSendLogSample(logDataBuffer,logDataLength);
    }
#endif 
}
