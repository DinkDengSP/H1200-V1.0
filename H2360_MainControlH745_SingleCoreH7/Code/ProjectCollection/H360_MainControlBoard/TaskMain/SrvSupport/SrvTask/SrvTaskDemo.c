...
** LastEditTime  : 2023-11-09 15:52:50 +0800
************************************************************************************************/ 
#include "SrvTaskHeader.h"
#include "SrvImplDemo.h"

//任务的控制块
TX_THREAD tcbSrvTaskDemo@".dtcm_ram";
//任务堆栈
uint64_t stackBufferSrvTaskDemo[STK_SIZE_SRV_TASK_DEMO/8]@".dtcm_ram";
//任务消息队列控制块
TX_QUEUE queueSrvTaskDemo@".dtcm_ram";
//消息缓冲区
uint32_t queueBufferSrvTaskDemo[LENGTH_QUEUE_SRV_TASK_DEMO]@".dtcm_ram";
...
void SrvTaskDemoFuncEntry(unsigned long threadInput)
{
    ...
    while(1)
    {
        ...
        //计算下一次调度延时
        if(timeFsmRunIntervalTimeMsUtil >= MS_TO_OS_TICK(SRV_TASK_DEMO_FSM_PERIOD_MS))
        {
        ...
        else 
        {
            ...
            nextTimeMsWaitQueueMsg = MS_TO_OS_TICK(SRV_TASK_DEMO_FSM_PERIOD_MS) - timeFsmRunIntervalTimeMsUtil;
        }
        //防溢出
