...
{
    stepMotorVarArray[STEP_MOTOR_NO2].var.motorMainState = mainState;
    stepMotorVarArray[STEP_MOTOR_NO2].var.motorSubState = subState;
}

//修改电机坐标
void IPC_SrvStepMotor2ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVarArray[STEP_MOTOR_NO2].var.currentCoordinate = motorCurrentCoordinate;
}
...
FlagStatus IPC_SrvStepMotor2CheckEmergencyStopCmd(void)
{
    if((stepMotorVarArray[STEP_MOTOR_NO2].var.motorMainState == STEP_MOTOR_MAIN_STOP)&&
        (stepMotorVarArray[STEP_MOTOR_NO2].var.motorSubState == STEP_MOTOR_STOP_SUB_EMERGENCY_CMD))
    {
        return SET;
    }
    return RESET;
