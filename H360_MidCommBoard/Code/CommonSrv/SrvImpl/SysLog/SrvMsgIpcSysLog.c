 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-02-01 17:18:12
...
/*************************************************************IPC指令处理处理***********************************************************/
//通讯测试
static void SrvMsgIpcSysLogCmd_CommTest(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);

//IPC指令处理函数列表
static const USER_IPC_CMD_PROC_UNIT ipcSysLogMsgCmdProcUnitArray[] = 
{
    //顺序是 指令码 预期参数最小长度  指令处理函数
    {(uint32_t)0XFFFFFFFF     ,0    ,SrvMsgIpcSysLogCmd_CommTest  },
};


//IPC指令消息处理函数
void SrvMsgIpcSysLogCmd(void* ipcMsgBodyPtr)
{
    //转换指令码
    ...
    //搜索匹配处理器
    uint16_t indexSysLog = 0;
    for(indexSysLog = 0; indexSysLog < DIM_ARRAY_ELEMENT_COUNT(ipcSysLogMsgCmdProcUnitArray);indexSysLog++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcSysLogMsgCmdProcUnitArray[indexSysLog].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexSysLog == DIM_ARRAY_ELEMENT_COUNT(ipcSysLogMsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcSysLogMsgCmdProcUnitArray[indexSysLog].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcSysLogMsgCmdProcUnitArray[indexSysLog].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcSysLogMsgCmdProcUnitArray[indexSysLog].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcSysLogMsgCmdProcUnitArray[indexSysLog].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
/*************************************************************IPC上报讯息处理**********************************************************/
//占位函数
static void SrvMsgIpcSysLogUpload_PlaceHolder(IPC_UPLOAD_DATA* ipcUploadDataPtr);

//IPC上传消息处理函数列表
static const USER_IPC_UPLOAD_PROC_UNIT ipcSysLogMsgUploadProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)0XFFFFFFFF     ,2 ,SrvMsgIpcSysLogUpload_PlaceHolder},
};


//IPC上传消息处理函数
void SrvMsgIpcSysLogUpload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexSysLog = 0;
    for(indexSysLog = 0; indexSysLog < DIM_ARRAY_ELEMENT_COUNT(ipcSysLogMsgUploadProcUnitArray);indexSysLog++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcSysLogMsgUploadProcUnitArray[indexSysLog].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexSysLog == DIM_ARRAY_ELEMENT_COUNT(ipcSysLogMsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcSysLogMsgUploadProcUnitArray[indexSysLog].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcSysLogMsgUploadProcUnitArray[indexSysLog].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcSysLogMsgUploadProcUnitArray[indexSysLog].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcSysLogMsgUploadProcUnitArray[indexSysLog].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
