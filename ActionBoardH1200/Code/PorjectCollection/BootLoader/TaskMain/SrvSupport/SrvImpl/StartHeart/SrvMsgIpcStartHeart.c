...
/*************************************************************IPC指令处理处理***********************************************************/
//通讯测试
static void SrvMsgIpcStartHeartCmd_CommTest(IPC_CMD_BASE_DATA* ipcCmdMsgPtr,SYS_RESULT_PACK* resultPackPtr,IPC_CHANNEL_CACHE* channelCachePtr);

//IPC指令处理函数列表
static const USER_IPC_CMD_PROC_UNIT ipcStartHeartMsgCmdProcUnitArray[] = 
{
    //顺序是 指令码 预期参数最小长度  指令处理函数
    {(uint32_t)0XFFFFFFFF     ,0    ,SrvMsgIpcStartHeartCmd_CommTest  },
};


//IPC指令消息处理函数
void SrvMsgIpcStartHeartCmd(void* ipcMsgBodyPtr)
{
    //转换指令码
    ...
    for(indexStartHeart = 0; indexStartHeart < DIM_ARRAY_ELEMENT_COUNT(ipcStartHeartMsgCmdProcUnitArray);indexStartHeart++)
    {
        if(ipcCmdMsgPtr->cmdBaseData.commandCode == (uint32_t)(ipcStartHeartMsgCmdProcUnitArray[indexStartHeart].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexStartHeart == DIM_ARRAY_ELEMENT_COUNT(ipcStartHeartMsgCmdProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength < ipcStartHeartMsgCmdProcUnitArray[indexStartHeart].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        ipcStartHeartMsgCmdProcUnitArray[indexStartHeart].exceptParamLengthMin,ipcCmdMsgPtr->cmdBaseData.cmdParamBufferLength,
    ...
    //检测函数指针
    if(ipcStartHeartMsgCmdProcUnitArray[indexStartHeart].ipcCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    ipcStartHeartMsgCmdProcUnitArray[indexStartHeart].ipcCmdProcFuncPtr(&(ipcCmdMsgPtr->cmdBaseData),&sysResultPack,ipcCmdMsgPtr->channelCachePtr);
    //处理完成回传
...
/*************************************************************IPC上报讯息处理**********************************************************/
//占位函数
static void SrvMsgIpcStartHeartUpload_PlaceHolder(IPC_UPLOAD_DATA* ipcUploadDataPtr);

//IPC上传消息处理函数列表
static const USER_IPC_UPLOAD_PROC_UNIT ipcStartHeartMsgUploadProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)0XFFFFFFFF     ,2 ,SrvMsgIpcStartHeartUpload_PlaceHolder},
};


//IPC上传消息处理函数
void SrvMsgIpcStartHeartUpload(void* ipcMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexStartHeart = 0;
    for(indexStartHeart = 0; indexStartHeart < DIM_ARRAY_ELEMENT_COUNT(ipcStartHeartMsgUploadProcUnitArray);indexStartHeart++)
    {
        if(ipcUploadDataPtr->uploadID == (uint32_t)(ipcStartHeartMsgUploadProcUnitArray[indexStartHeart].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexStartHeart == DIM_ARRAY_ELEMENT_COUNT(ipcStartHeartMsgUploadProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(ipcUploadDataPtr->dataLength < ipcStartHeartMsgUploadProcUnitArray[indexStartHeart].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        ipcStartHeartMsgUploadProcUnitArray[indexStartHeart].exceptParamLengthMin,ipcUploadDataPtr->dataLength,
    ...
    //检测函数指针
    if(ipcStartHeartMsgUploadProcUnitArray[indexStartHeart].ipcUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    ipcStartHeartMsgUploadProcUnitArray[indexStartHeart].ipcUploadProcFuncPtr(ipcUploadDataPtr);
    //处理完成,释放内存
    if((ipcUploadDataPtr->dataBufferPtr != NULL)&&(ipcUploadDataPtr->dataLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_IPC,ipcUploadDataPtr->dataBufferPtr);
