...
//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_SRV_STEP_MOTOR9             128
//IPC通道讯息
...
{
    stepMotorVar[STEP_MOTOR_NO9]->var.motorMainState = mainState;
    stepMotorVar[STEP_MOTOR_NO9]->var.motorSubState = subState;
}
...
void IPC_SrvStepMotor9EmergencyStop(void)
{
    ...
    stepMotorVar[STEP_MOTOR_NO9]->var.motorSubState = STEP_MOTOR_STOP_SUB_EMERGENCY_CMD;
    //步进电机写入急停
    BoardCPLD_StepMotorEmergencyStop(BOARD_CPLD_MOTOR9);
}

//修改电机坐标
void IPC_SrvStepMotor9ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVar[STEP_MOTOR_NO9]->var.currentCoordinate = motorCurrentCoordinate;
}
...
{
    return stepMotorVar[STEP_MOTOR_NO9]->var.currentCoordinate;
}
