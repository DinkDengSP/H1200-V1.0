...
//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_SRV_STEP_MOTOR6             128
//IPC通道讯息
...
void IPC_SrvStepMotor5Init(void)
{
    IPC_BaseInitChannelData(&ipcChannelDataSrvStepMotor5,(uint8_t*)ipcChannelSrvStepMotor5NameStr,
                                &queueSrvTaskStepMotor5,LENGTH_BUFFER_IPC_SRV_STEP_MOTOR6);
}
...
{
    stepMotorVar[STEP_MOTOR_NO5]->var.motorMainState = mainState;
    stepMotorVar[STEP_MOTOR_NO5]->var.motorSubState = subState;
}
...
void IPC_SrvStepMotor5EmergencyStop(void)
{
    ...
    stepMotorVar[STEP_MOTOR_NO5]->var.motorSubState = STEP_MOTOR_STOP_SUB_EMERGENCY_CMD;
    //步进电机写入急停
    BoardCPLD_StepMotorEmergencyStop(BOARD_CPLD_MOTOR5);
}

//修改电机坐标
void IPC_SrvStepMotor5ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVar[STEP_MOTOR_NO5]->var.currentCoordinate = motorCurrentCoordinate;
}

//获取电机实时坐标
int32_t IPC_SrvStepMotor5GetRealTimeCoordinate(void)
{
    return stepMotorVar[STEP_MOTOR_NO5]->var.currentCoordinate;
}
