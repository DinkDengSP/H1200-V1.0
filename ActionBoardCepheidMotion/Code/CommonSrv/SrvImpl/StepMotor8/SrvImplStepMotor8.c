...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor8Cmd(void* canMsgBodyPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor8Upload(void* canMsgBodyPtr);
//CAN2指令消息处理函数
extern void SrvMsgCan2StepMotor8Cmd(void* canMsgBodyPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2StepMotor8Upload(void* canMsgBodyPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor8Cmd(void* ipcMsgBodyPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor8Upload(void* ipcMsgBodyPtr);
...
void SrvImplStepMotor8MsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_CMD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor8Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_UPLOAD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor8Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
