...
//传输正在进行标志
MCU_UART_TRANS_STATE mcuUart2TransState = MCU_UART_TRANS_IDLE;
#endif
//中断发送,需要使用的数据块
...
//接收缓存
static UART_RECV_PACK uart2PackRecvArray[COUNT_UART2_RECV_BLOCK];
//接收缓存
...
static void MCU_UART2_BaseInit(uint32_t baud, MCU_UART_LENGTH length, MCU_UART_STOPBIT stopBit,MCU_UART_CHECK_MODE checkMode, MCU_UART_HARD_CONTROL hardWareControl)
{
    ...
    //清除TC发送完成标志 
    SET_BIT(USART2->SR, UART_FLAG_TC);
    //清除RXNE接收FIFO,清空发送之后需要延时一会,等待清空之后再打开空闲中断标记,否则一打开中断就会收到空闲中断
    SET_BIT(USART2->CR1, UART_IT_IDLE);
    //等待清除完成
    CoreDelayMinTick();
...
#if((MCU_UART2_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART2_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
    //数据结构初始化
    circularByteQueueUart2SendPtr = ADT_CircularByteQueueCreate(LENGTH_UART2_BUFFER_WITH_SEND,MCU_MemMallocSramIn,MCU_MemFreeSramIn,
...
void MCU_UART2_TransDMA_Init(void)
{
    ...
    //通道设定
    UART2_TxDMA_Handler.Init.Channel = MCU_UART2_DMA_TX_CHANNEL; 
    /*方向：从内存到外设*/		
...
void MCU_UART2_RecvDataInit(void)
{
    ...
    //初始化接收队列
    circularPtrQueueUart2RecvIdleBlockPtr = ADT_CircularPtrQueueCreate(COUNT_UART2_RECV_BLOCK,MCU_MemMallocSramIn,MCU_MemFreeSramIn,
    ...
    {
        uart2PackRecvArray[indexUtil].packSrc = UART_PACK_SRC_2;
        uart2PackRecvArray[indexUtil].uartRecvDataBuffer = (uint8_t*)(blockBufferWithUart2Recv[indexUtil]);
        uart2PackRecvArray[indexUtil].uartRecvDataLength = 0;
    }
    //队列导入
    for(indexUtil = 0;indexUtil < COUNT_UART2_RECV_BLOCK;indexUtil++)
    {
        ADT_CircularPtrQueueWrite(circularPtrQueueUart2RecvIdleBlockPtr,(void*)(&(uart2PackRecvArray[indexUtil])));
    }
    //清除当前接收队列
...
void MCU_UART2_RecvDMA_Init(void)
{
    ...
    //通道号
    UART2_RxDMA_Handler.Init.Channel = MCU_UART2_DMA_RX_CHANNEL; 
    /*方向：从外设到内存*/		
...
//串口接收到的数据释放
void MCU_UART2_RecvDatRelease(UART_RECV_PACK* uartPackRecvPtr)
{
    asm("nop");
#if((MCU_UART2_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART2_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
    ...
    UserMemClear((void*)(uartPackRecvPtr->uartRecvDataBuffer),LENGTH_UART2_RECV_SINGLE_BLOCK);
    //数据指针写回
    ADT_CircularPtrQueueWrite(circularPtrQueueUart2RecvIdleBlockPtr,uartPackRecvPtr);
#endif
}
...
static void MCU_UART2_RxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    ...
    else
    {
        ...
        recvDispatchPtr->uartRecvDataLength = LENGTH_UART2_RECV_SINGLE_BLOCK;
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->uartRecvDataBuffer),(void*)currentBufferWithUart2Recv,LENGTH_UART2_RECV_SINGLE_BLOCK);
        //清空接收
        UserMemClear((void*)currentBufferWithUart2Recv,LENGTH_UART2_RECV_SINGLE_BLOCK);
#if(MCU_UART2_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
        //该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
...
static void MCU_UART2_RxEventCallBack(UART_HandleTypeDef* uartHandlerPtr,uint16_t dataLength)
{
    ...
    else
    {
        ...
        //数据拷贝
        UserMemCopy((void*)(recvDispatchPtr->uartRecvDataBuffer),(void*)currentBufferWithUart2Recv,dataLength);
        //清空接收
        UserMemClear((void*)currentBufferWithUart2Recv,LENGTH_UART2_RECV_SINGLE_BLOCK);
        //打开空闲接收
        HAL_UARTEx_ReceiveToIdle_IT(&UART2_Handler,(uint8_t *)(currentBufferWithUart2Recv), LENGTH_UART2_RECV_SINGLE_BLOCK);
