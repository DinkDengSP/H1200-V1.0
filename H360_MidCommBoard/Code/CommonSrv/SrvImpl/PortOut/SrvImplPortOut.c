 ...
 ** description  : 
 ** LastEditors  : 邓小俊
 ** LastEditTime : 2024-02-02 14:05:01
...
//网口指令消息处理函数
extern void SrvMsgNet8700PortOutCmd(void* net8700CmdRecvPtr);
//网口上传消息处理函数
extern void SrvMsgNet8700PortOutUpload(void* net8700ReportDataPtr);
//CAN1指令消息处理函数
extern void SrvMsgCan1PortOutCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1PortOutUpload(void* can1ReportDataPtr);
//CAN2指令消息处理函数
extern void SrvMsgCan2PortOutCmd(void* can2CmdRecvPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2PortOutUpload(void* can2ReportDataPtr);
//串口指令消息处理函数
extern void SrvMsgUartPortOutCmd(void* uartCmdRecvPtr);
//串口上传消息处理函数
extern void SrvMsgUartPortOutUpload(void* uartReportDataPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcPortOutCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcPortOutUpload(void* ipcReportDataPtr);
...
void SrvImplPortOutMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_NET_CMD)
    {
        ...
        {
            //调用网口消息处理函数
            SrvMsgNet8700PortOutCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_NET_UPLOAD)
    {
        ...
        {
            //调用网口消息处理函数
            SrvMsgNet8700PortOutUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_CMD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartPortOutCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_UPLOAD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartPortOutUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
