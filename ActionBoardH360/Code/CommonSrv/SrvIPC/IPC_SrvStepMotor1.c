...
//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_SRV_STEP_MOTOR1             128
//IPC通道讯息
...
{
    stepMotorVar[STEP_MOTOR_NO1]->var.motorMainState = mainState;
    stepMotorVar[STEP_MOTOR_NO1]->var.motorSubState = subState;
}
...
void IPC_SrvStepMotor1EmergencyStop(void)
{
    ...
    stepMotorVar[STEP_MOTOR_NO1]->var.motorSubState = STEP_MOTOR_STOP_SUB_EMERGENCY_CMD;
    //步进电机写入急停
    BoardCPLD_StepMotorEmergencyStop(BOARD_CPLD_MOTOR1);
}

//修改电机坐标
void IPC_SrvStepMotor1ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVar[STEP_MOTOR_NO1]->var.currentCoordinate = motorCurrentCoordinate;
}

//获取电机实时坐标
int32_t IPC_SrvStepMotor1GetRealTimeCoordinate(void)
{
    return stepMotorVar[STEP_MOTOR_NO1]->var.currentCoordinate;
}
