...
//传输正在进行标志
static MCU_UART_TRANS_STATE mcuUart1TransState = MCU_UART_TRANS_IDLE;
#endif
//中断发送,需要使用的数据块
...
#pragma pack(4)
//用于当前发送的数据块
static uint8_t blockBufferWithUart1SendCurrent[LENGTH_UART1_BUFFER_WITH_BLOCK]@".d3_sram4_dma" = {0};
#pragma pack (pop)
#endif
...
//当前接收对象
static uint8_t currentBufferWithUart1Recv[LENGTH_UART1_RECV_SINGLE_BLOCK]@".d3_sram4_dma" = {0};
#endif  
...
static void MCU_UART1_BaseInit(uint32_t baud, MCU_UART_LENGTH length, MCU_UART_STOPBIT stopBit,MCU_UART_CHECK_MODE checkMode, MCU_UART_HARD_CONTROL hardWareControl)
{
    ...
    //是否关闭ORE过载标记
#if(MCU_UART1_DISABLE_ORE == CONFIG_FUNC_ENABLE)
...
#if((MCU_UART1_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART1_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
    //数据结构初始化
    circularByteQueueUart1SendPtr = ADT_CircularByteQueueCreate(LENGTH_UART1_BUFFER_WITH_SEND,MCU_MemMallocAxiSram,MCU_MemFreeAxiSram,
...
void MCU_UART1_RecvDataInit(void)
{
    ...
    //初始化接收队列
    circularPtrQueueUart1RecvIdleBlockPtr = ADT_CircularPtrQueueCreate(COUNT_UART1_RECV_BLOCK,MCU_MemMallocAxiSram,MCU_MemFreeAxiSram,
    ...
    {
        uart1RecvPackArray[indexUtil].packSrc = UART_PACK_SRC_1;
    ...
    for(indexUtil = 0;indexUtil < COUNT_UART1_RECV_BLOCK;indexUtil++)
    {
        ADT_CircularPtrQueueWrite(circularPtrQueueUart1RecvIdleBlockPtr,(void*)(&(uart1RecvPackArray[indexUtil])));
    }
    //清除当前接收队列
...
//是否异步发送
#if(UART1_TRANS_FUNC_SYNC == CONFIG_FUNC_ENABLE) 
    //等待发送完成
...
void MCU_UART1_RecvDatRelease(UART_RECV_PACK* uartRecvPackPtr)
{
    ...
    //数据指针写回
    ADT_CircularPtrQueueWrite(circularPtrQueueUart1RecvIdleBlockPtr,uartRecvPackPtr);
#endif
}
...
//串口回调函数,接收完成中断
void MCU_UART1_RxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    asm("nop");
#if((MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)||(MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_DMA))
...
//串口回调函数,发送完成中断
void MCU_UART1_TxCpltCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    asm("nop");
#if((MCU_UART1_TX_FUNC_USER == MCU_UART_TX_FUNC_INT)||(MCU_UART1_TX_FUNC_USER == MCU_UART_TX_FUNC_DMA))
...
//串口回调函数,错误中断
void MCU_UART1_ErrorCallBack(UART_HandleTypeDef* uartHandlerPtr)
{
    //关闭接收
...
//串口回调函数,接收事件中断
void MCU_UART1_RxEventCallBack(UART_HandleTypeDef* uartHandlerPtr,uint16_t dataLength)
{
#if(MCU_UART1_RX_FUNC_USER == MCU_UART_RX_FUNC_INT)
