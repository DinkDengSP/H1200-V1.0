...
//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_SRV_STEP_MOTOR12             128
//IPC通道讯息
...
{
    stepMotorVar[STEP_MOTOR_NO12]->var.motorMainState = mainState;
    stepMotorVar[STEP_MOTOR_NO12]->var.motorSubState = subState;
}
...
void IPC_SrvStepMotor12EmergencyStop(void)
{
    ...
    stepMotorVar[STEP_MOTOR_NO12]->var.motorSubState = STEP_MOTOR_STOP_SUB_EMERGENCY_CMD;
    //步进电机写入急停
    BoardCPLD_StepMotorEmergencyStop(BOARD_CPLD_MOTOR12);
}

//修改电机坐标
void IPC_SrvStepMotor12ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVar[STEP_MOTOR_NO12]->var.currentCoordinate = motorCurrentCoordinate;
}
...
{
    return stepMotorVar[STEP_MOTOR_NO12]->var.currentCoordinate;
}
