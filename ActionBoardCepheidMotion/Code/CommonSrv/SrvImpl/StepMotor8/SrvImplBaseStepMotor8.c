                                        ...
                                        uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = UserMemMallocWhileSuccess(MEM_REGION_SRV_TASK,*dataBufLen);
    ...
    //读取当前电机坐标
    int32_t motorCurrentCoordinate = stepMotorVar[STEP_MOTOR_NO8]->var.currentCoordinate;
    //读取电机当前编码器坐标
    int32_t motorCurrentEncoder = 0;
    //读取电机传感器状态
    PIN_STATE originSensorState,emStopSensorState,utilStop1SensorState,utilStop2SensorState;
    StepMotorReadSensorState(STEP_MOTOR_NO8,&originSensorState,&emStopSensorState,&utilStop1SensorState,
...
//电机回零            
void SrvImplBaseStepMotor8ReturnZero(StepMotorReturnZeroCmd* returnZeroCmdPtr,uint8_t needReset,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,
                                        uint16_t* dataBufLen)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //返回内存长度设定
...
void SrvImplBaseStepMotor8StopNow(SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    //立即停止
    errorSub = StepMotorStopImmediately(STEP_MOTOR_NO8);
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
        CoreUtilSetSystemResultPack(resultPackPtr,PROT_RESULT_FAIL,ERROR_MAIN_ACT_STEP_MOTOR,ERROR_LEVEL_ERROR,
                                ...
                                uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = UserMemMallocWhileSuccess(MEM_REGION_SRV_TASK,*dataBufLen);
    ...
    {
        //慢速
        errorSub = StepMotorStopDeceleration(STEP_MOTOR_NO8);
    }
    else
    {
        //快速
