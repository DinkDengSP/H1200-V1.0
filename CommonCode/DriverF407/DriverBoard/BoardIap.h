...
** LastEditTime  : 2023-10-25 14:58:49 +0800
************************************************************************************************/ 
...
//计算CRC32
uint32_t BoardIapCalcCRC32Once(const uint8_t *pu8Data, uint32_t u32Size);
//计算CRC32,带记录过程计算
uint32_t BoardIapCalcCRC32OnceRecordNotEnd(const uint8_t *pu8Data, uint32_t u32Size,uint32_t crcRecord);
//计算CRC32,带记录最后一包计算
uint32_t BoardIapCalcCRC32OnceRecordEnd(const uint8_t *pu8Data, uint32_t u32Size,uint32_t crcRecord);
...
//读取 SRV IAP升级记录信息
ERROR_SUB BoardIapReadSrvRecord(BOARD_IAP_RECODER* recordMsgPtr);
//写入 SRV IAP记录信息
ERROR_SUB BoardIapWriteSrvRecord(BOARD_IAP_RECODER* recordMsgPtr);
//初始化 SRV IAP记录信息
ERROR_SUB BoardIapWriteSrvRecordDefault(void);
