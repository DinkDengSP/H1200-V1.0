...
** LastEditTime  : 2023-11-29 17:56:32 +0800
************************************************************************************************/ 
#include "H360SysPortIn.h"
#include "SysState.h"

//获取板号和模块号
static ERROR_SUB H360SysPortInGetModuleBoard(H360_SYS_PORT_IN portIndex,uint8_t* moduleNo,uint8_t* boardID,
                                            uint8_t* convertPinIndex,FlagStatus* isSelfBoard)
{
    *isSelfBoard = RESET;
    ...
    {
        //主控板
        *moduleNo = Can2ProtPortGetSelfModuleNo();
        *boardID = Can2ProtPortGetSelfBoardID();
...
ERROR_SUB H360SysPortInRead(H360_SYS_PORT_IN portIndex,GPIO_PinState* readStatePtr)
{
    ...
    //识别端口号
    errorSub = H360SysPortInGetModuleBoard(portIndex,&moduleNo,&boardID,&convertPinIndex,&isSelfBoard);
