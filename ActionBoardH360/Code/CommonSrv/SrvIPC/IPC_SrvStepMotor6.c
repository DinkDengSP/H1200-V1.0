...
void IPC_SrvStepMotor6Init(void)
{
    IPC_BaseInitChannelData(&ipcChannelDataSrvStepMotor6,(uint8_t*)ipcChannelSrvStepMotor6NameStr,
                                &queueSrvTaskStepMotor6,LENGTH_BUFFER_IPC_SRV_STEP_MOTOR5);
}
...
{
    stepMotorVar[STEP_MOTOR_NO6]->var.motorMainState = mainState;
    stepMotorVar[STEP_MOTOR_NO6]->var.motorSubState = subState;
}
...
void IPC_SrvStepMotor6EmergencyStop(void)
{
    ...
    stepMotorVar[STEP_MOTOR_NO6]->var.motorSubState = STEP_MOTOR_STOP_SUB_EMERGENCY_CMD;
    //步进电机写入急停
    BoardCPLD_StepMotorEmergencyStop(BOARD_CPLD_MOTOR6);
}

//修改电机坐标
void IPC_SrvStepMotor6ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVar[STEP_MOTOR_NO6]->var.currentCoordinate = motorCurrentCoordinate;
}

//获取电机实时坐标
int32_t IPC_SrvStepMotor6GetRealTimeCoordinate(void)
{
    return stepMotorVar[STEP_MOTOR_NO6]->var.currentCoordinate;
}
