...
** LastEditTime  : 2023-09-15 15:41:33 +0800
************************************************************************************************/ 
#include "MCU_SPI1.h"
#include "MCU_PIN.h"
...
//保存配置
static SPI123_CONFIG spi1Config;
//互斥量
TX_MUTEX mutexMCU_SPI1;
//SPI1操作句柄
...
static void MCU_SPI1_MspInit(void)
{
    ...
    __HAL_RCC_SPI1_CLK_DISABLE();
    //选择时钟端口
    RCC_PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_SPI1;
...
//MCU SPI基础初始化
static void MCU_SPI1_BaseConfig(MCU_SPI_123_CLK speed,MCU_SPI_CPOL cpol,MCU_SPI_CPHA cpha)
{
	...
	SPI1_Handler.Init.CRCLength         = SPI_CRC_LENGTH_8BIT;
	SPI1_Handler.Init.NSS               = SPI_NSS_SOFT;
    SPI1_Handler.Init.NSSPMode          = SPI_NSS_PULSE_DISABLE;
    SPI1_Handler.Init.NSSPolarity       = SPI_NSS_POLARITY_LOW;
    ...
    SPI1_Handler.Init.MasterInterDataIdleness = SPI_MASTER_INTERDATA_IDLENESS_02CYCLE;
    //插入到NSS有效边沿和第一个数据开始之间的额外延迟，单位SPI时钟周期个数
    SPI1_Handler.Init.MasterSSIdleness = SPI_MASTER_SS_IDLENESS_00CYCLE;
    //模式
...
#pragma pack(4)
//用于DMA发送数据快
static uint8_t bufferWithSpi1Send[LENGTH_BYTES_SPI1_DMA_BUF]@".axi_dma_sram" = {0};
//用于DMA接收数据块
static uint8_t bufferWithSpi1Recv[LENGTH_BYTES_SPI1_DMA_BUF]@".axi_dma_sram" = {0};
#pragma pack (pop)
...
static void MCU_SPI1_DMA_Init(void)
{
    ...
    SPI1_DMA_TX_Handler.Init.Request             = DMA_REQUEST_SPI1_TX;
    //内存到外设
    ...
    SPI1_DMA_TX_Handler.Init.MemBurst            = DMA_MBURST_SINGLE;	
    SPI1_DMA_TX_Handler.Init.PeriphBurst         = DMA_PBURST_SINGLE;	
    ...
    halState = HAL_DMA_Init(&SPI1_DMA_TX_Handler);
    if(halState != HAL_OK)
    {
        MCU_LogShowHalState(halState,SPI1_DMA_TX_Handler.ErrorCode);
        while(1);
    }
    //发送DMA中断设定
    ...
    SPI1_DMA_RX_Handler.Init.Request             = DMA_REQUEST_SPI1_RX;
    //外设到内存
    ...
    halState = HAL_DMA_Init(&SPI1_DMA_RX_Handler);    
    if(halState != HAL_OK)
    {
        MCU_LogShowHalState(halState,SPI1_DMA_RX_Handler.ErrorCode);
        while(1);
    }
    //接收DMA中断设定
...
ERROR_SUB MCU_SPI1_ReadWriteBuffer(uint8_t* writeDataPtr,uint8_t* readDataPtr,uint16_t dataLength)
{
    ...
    {
        UserMemSet(bufferWithSpi1Send,0,LENGTH_BYTES_SPI1_DMA_BUF);
    }
    //获取锁
    MCU_SPI1_GetLock();
    for(indexUtil = 0; indexUtil < sendPackCountMax;indexUtil++)
    {
        ...
        {
            //接收数据清空
            UserMemSet(bufferWithSpi1Recv,0,LENGTH_BYTES_SPI1_DMA_BUF);
        }
        //清空信号量标记
        ...
        //DMA数据发送,同时收发
        halState = HAL_SPI_TransmitReceive_DMA(&SPI1_Handler, (uint8_t*)bufferWithSpi1Send, (uint8_t *)bufferWithSpi1Recv, 
                                                    singlePackLength);
        if(halState != HAL_OK)
        {
            //释放锁
        ...
        if(readDataPtr != SPI_PTR_NULL)
        {
            ...
            //SCB_InvalidateDCache_by_Addr((uint32_t *)bufferWithSpi1Recv, LENGTH_BYTES_SPI1_DMA_BUF);
            //完成之后进行接收数据拷贝
