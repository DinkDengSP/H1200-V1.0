...
//CAN1指令消息处理函数
extern void ModMsgCan1HandWashCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void ModMsgCan1HandWashUpload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void ModMsgIpcHandWashCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void ModMsgIpcHandWashUpload(void* ipcReportDataPtr);
...
void ModImplHandWashMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1HandWashCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1HandWashUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcHandWashCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcHandWashUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
