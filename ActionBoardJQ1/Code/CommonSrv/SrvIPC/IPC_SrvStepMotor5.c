...
//通道携带的数据缓存长度
#define LENGTH_BUFFER_IPC_SRV_STEP_MOTOR5             128
//IPC通道讯息
...
{
    stepMotorVarArray[STEP_MOTOR_NO5].var.motorMainState = mainState;
    stepMotorVarArray[STEP_MOTOR_NO5].var.motorSubState = subState;
}

//修改电机坐标
void IPC_SrvStepMotor5ModifyMotorCordinate(int32_t motorCurrentCoordinate)
{
    stepMotorVarArray[STEP_MOTOR_NO5].var.currentCoordinate = motorCurrentCoordinate;
}

//检查电机是否指令急停
FlagStatus IPC_SrvStepMotor5CheckEmergencyStopCmd(void)
{
    if((stepMotorVarArray[STEP_MOTOR_NO5].var.motorMainState == STEP_MOTOR_MAIN_STOP)&&
        (stepMotorVarArray[STEP_MOTOR_NO5].var.motorSubState == STEP_MOTOR_STOP_SUB_EMERGENCY_CMD))
    {
        return SET;
    }
    return RESET;
...
{
    if((stepMotorVarArray[STEP_MOTOR_NO5].var.motorMainState == STEP_MOTOR_MAIN_STOP)&&
        (stepMotorVarArray[STEP_MOTOR_NO5].var.motorSubState == STEP_MOTOR_STOP_SUB_EMERGENCY_SIGNAL))
    {
        return SET;
    }
    return RESET;
