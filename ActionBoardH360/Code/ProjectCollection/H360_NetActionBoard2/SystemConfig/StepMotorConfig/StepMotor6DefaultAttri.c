...
** LastEditTime  : 2024-01-05 13:07:00 +0800
************************************************************************************************/ 
#include "StepMotorParam.h"
//步进电机6:  旧杯盘升降步进电机(描述电机名称)
//步进电机默认配置
const STEP_MOTOR_ATTRIBUTE StepMotor6DefaultAttri = {
    ...
    //基础配置
    {
        ...
        20000,//uint32_t resetMaxSteps;电机复位最大行程
        0,//uint32_t resetDaccSteps,到零点后减速停止的步数,不能挡片遮挡原点传感器的间隙步数
        0,//uint32_t resetOutSensorSteps 复位时脱离零点运行的步数
        -1000,//int32_t reverseMaxSteps;反向最大坐标
        20000,//int32_t positiveMaxSteps;正向最大坐标
        100,//uint32_t dacc2StopSteps;减速停止的减速步数
        50.0f,//float resetSpeedFast;复位速度,快速,主要用在找原点
        30.0f,//float resetSpeedSlow;复位速度,慢速,主要用在脱离原点和二次回到原点
