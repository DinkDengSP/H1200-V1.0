...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor12Cmd(void* canMsgBodyPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor12Upload(void* canMsgBodyPtr);
//CAN2指令消息处理函数
extern void SrvMsgCan2StepMotor12Cmd(void* canMsgBodyPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2StepMotor12Upload(void* canMsgBodyPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor12Cmd(void* ipcMsgBodyPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor12Upload(void* ipcMsgBodyPtr);
...
void SrvImplStepMotor12MsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_CMD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor12Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_UPLOAD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor12Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
