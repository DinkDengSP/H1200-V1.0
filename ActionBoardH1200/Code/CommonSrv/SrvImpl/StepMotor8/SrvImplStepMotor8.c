...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor8Cmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor8Upload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor8Cmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor8Upload(void* ipcReportDataPtr);
//串口指令消息处理函数
extern void SrvMsgUartStepMotor8Cmd(void* uartCmdRecvPtr);
//串口上传消息处理函数
extern void SrvMsgUartStepMotor8Upload(void* uartReportDataPtr);
...
void SrvImplStepMotor8MsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            SrvMsgCan1StepMotor8Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            SrvMsgCan1StepMotor8Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            SrvMsgIpcStepMotor8Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            SrvMsgIpcStepMotor8Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_CMD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartStepMotor8Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_UART_UPLOAD)
    {
        ...
        {
            //调用串口消息处理函数
            SrvMsgUartStepMotor8Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
