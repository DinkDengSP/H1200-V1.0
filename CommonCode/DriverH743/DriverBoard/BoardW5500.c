...
** Date          : 2023-03-10 09:04:40 +0800
** Description   : When I Has Time ,I Will Write Description Below:
** ModifyRecord1 :    
** ModifyRecord2 :    
** LastEditors   : DengXiaoJun(邓小俊)
** LastEditTime  : 2023-07-20 16:56:05 +0800
************************************************************************************************/ 
...
void BoardW5500_ReleaseRecvDataBuffer(uint8_t *recvDataBufferPtr)
{
    ...
    // 释放内存
    UserMemFree(BOARD_W5500_USE_MEM_REGION, recvDataBufferPtr);
}
...
ERROR_SUB BoardW5500_ConfigAllTxRxBufferSize(void)
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    // 设置接收缓冲
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_0, BOARD_W5500_SOCKET_0_RX_BUF_SIZE);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_1, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_2, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_3, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_4, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_5, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_6, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketRxMemSizeReg(W5500_SOCKET_NO_7, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    // 设置发送缓冲
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_0, BOARD_W5500_SOCKET_0_TX_BUF_SIZE);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_1, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_2, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_3, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_4, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_5, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_6, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
    ...
    errorSub = BoardW5500_WriteSocketTxMemSizeReg(W5500_SOCKET_NO_7, BOARD_W5500_SOCKET_BUF_SIZE_0KB);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
...
{
    ERROR_SUB errorSub = ERROR_SUB_OK;
    errorSub = BoardW5500_WriteBaseSourceMacAddrReg(BOARD_W5500_MAC_ADDR_0, BOARD_W5500_MAC_ADDR_1, BOARD_W5500_MAC_ADDR_2, 
                                                       BOARD_W5500_MAC_ADDR_3, BOARD_W5500_MAC_ADDR_4, BOARD_W5500_MAC_ADDR_5);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
...
ERROR_SUB BoardW5500_ConfigSubNetMaskDefault(void)
{
    ...
    subNetMaskVal += BOARD_W5500_SUB_NET_MASK1;
    subNetMaskVal <<= 8;
    subNetMaskVal += BOARD_W5500_SUB_NET_MASK2;
    subNetMaskVal <<= 8;
    subNetMaskVal += BOARD_W5500_SUB_NET_MASK3;
    subNetMaskVal <<= 8;
    subNetMaskVal += BOARD_W5500_SUB_NET_MASK4;
...
static ERROR_SUB BoardW5500_WaitSendOverMacRaw(uint32_t timeOutMs)
{
    ...
    do
    {
        ...
        if ((utilTempByte & W5500_REG_VAL_SOCKET_Sn_IR_TIMEOUT) != 0)
        {
            ...
            BoardW5500_WriteSockeIntReg(W5500_SOCKET_NO_0, 0XFF);
            // 打印错误代码
            MCU_LogShowErrorSubCode(ERROR_SUB_BOARD_W5500_SEND_IR_TIME_OUT);
            return ERROR_SUB_BOARD_W5500_SEND_IR_TIME_OUT;
        }
        if (tx_time_get() - startWaitTime > MS_TO_OS_TICK(timeOutMs))
        {
            // 释放互斥信号量
            BoardW5500_ReleaseLock();
            // 清除中断
            BoardW5500_WriteSockeIntReg(W5500_SOCKET_NO_0, 0XFF);
            // 打印错误代码
            MCU_LogShowErrorSubCode(ERROR_SUB_BOARD_W5500_SEND_USER_TIME_OUT);
...
ERROR_SUB BoardW5500_SendOneFrameMacRaw(uint8_t *dataBufferPtr, uint16_t dataLength)
{
    ...
    do
    {
        ...
        if (utilTempByte == W5500_REG_VAL_SOCKET_Sn_SR_SOCK_CLOSED)
        {
            ...
            BoardW5500_ReleaseLock();
            // 打印错误代码
            MCU_LogShowErrorSubCode(ERROR_SUB_BOARD_W5500_SEND_CHECK_CLOSED);
    ...
    // 等待发送完成
    errorSub = BoardW5500_WaitSendOverMacRaw(BOARD_W5500_SEND_TIME_OUT_MAX);
    if (errorSub != ERROR_SUB_OK)
    {
        // 打印错误代码
        MCU_LogShowErrorSubCode(errorSub);
...
ERROR_SUB BoardW5500_RecvOneFrameMacRaw(uint8_t **dataBufferPtr, uint16_t *dataLength)
{
    ...
    *dataBufferPtr = UserMemMallocWhileSuccess(BOARD_W5500_USE_MEM_REGION, frameLength);
    // 再次读取数据
    errorSub = BoardW5500_ReadSocketRxDataBuffer(W5500_SOCKET_NO_0, *dataBufferPtr, frameLength);
    if (errorSub != ERROR_SUB_OK)
    {
        // 释放内存
        UserMemFree(BOARD_W5500_USE_MEM_REGION, *dataBufferPtr);
        *dataBufferPtr = NULL;
        *dataLength = 0;
        // 释放互斥信号量
        BoardW5500_ReleaseLock();
