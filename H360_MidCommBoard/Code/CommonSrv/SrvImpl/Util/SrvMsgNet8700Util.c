...
/****************************************网口指令消息处理函数**************************************************/
//占位函数
static void SrvMsgNet8700UtilCmd_PlaceHolder(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//联机
static void SrvMsgNetUtilCmd_OnLine(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//脱机
static void SrvMsgNetUtilCmd_OffLine(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//UTC同步
static void SrvMsgNetUtilCmd_UtcSync(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//心跳获取
static void SrvMsgNetUtilCmd_HeartGet(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//急停
static void SrvMsgNetUtilCmd_EmStop(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//获取版本号
static void SrvMsgNetUtilCmd_GetVersion(NET_PROT_CMD_PACK_RECV* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);

//NET指令处理函数列表
static const USER_NET_CMD_PROC_UNIT net8700CmdProcUtilUnitArray[] = 
{
    //顺序: 指令码  预期参数最小长度 是否需要结果返回  错误是否独立上报 指令处理函数
    {(uint32_t)NET_MASTER_CMD_HOLD_PLACE                 ,0  ,ENABLE ,SrvMsgNet8700UtilCmd_PlaceHolder   },
    {(uint32_t)NET_MASTER_CMD_MAIN_UTIL_ONLINE           ,1  ,ENABLE ,SrvMsgNetUtilCmd_OnLine               },
    {(uint32_t)NET_MASTER_CMD_MAIN_UTIL_OFFLINE          ,1  ,ENABLE ,SrvMsgNetUtilCmd_OffLine              },
    {(uint32_t)NET_MASTER_CMD_MAIN_UTIL_UTC_SYNC         ,8  ,ENABLE ,SrvMsgNetUtilCmd_UtcSync              },
    {(uint32_t)NET_MASTER_CMD_MAIN_UTIL_HEART_GET        ,0  ,ENABLE ,SrvMsgNetUtilCmd_HeartGet             },
    {(uint32_t)NET_MASTER_CMD_MAIN_UTIL_EM_STOP          ,0  ,ENABLE ,SrvMsgNetUtilCmd_EmStop               },
    {(uint32_t)NET_MASTER_CMD_MAIN_UTIL_GET_VERSION      ,0  ,ENABLE ,SrvMsgNetUtilCmd_GetVersion           },
};

void SrvMsgNet8700UtilCmd(void* net8700CmdRecvPtr)
{
    //转换指令码
    ...
    //接收打印显示
    SystemPrintf("Util Recv Net8700 Cmd: 0X%08X,DataLen: %d\r\n",cmdPackPtr->cmdCode,cmdPackPtr->dataBodyLength);
    //搜索匹配处理器
...
//NET上传消息处理函数列表
static const USER_NET_UPLOAD_PROC_UNIT netUploadUtilProcUnitArray[] = 
{
    //顺序是: 指令码 预期参数最小长度 上传消息处理函数
    {(uint32_t)NET_MASTER_REPORT_HOLD_PLACE     ,2 ,SrvMsgNet8700UtilUpload_PlaceHolder},
};

void SrvMsgNet8700UtilUpload(void* net8700ReportDataPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexUtil = 0;
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(netUploadUtilProcUnitArray);indexUtil++)
    {
        if(netUploadPackRecvPtr->uploadID == (uint32_t)(netUploadUtilProcUnitArray[indexUtil].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(netUploadUtilProcUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(netUploadPackRecvPtr->paramBufferLength < netUploadUtilProcUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        netUploadUtilProcUnitArray[indexUtil].exceptParamLengthMin,netUploadPackRecvPtr->paramBufferLength,
    ...
    //检测函数指针
    if(netUploadUtilProcUnitArray[indexUtil].netUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    netUploadUtilProcUnitArray[indexUtil].netUploadProcFuncPtr(netUploadPackRecvPtr);
    //处理完成,释放内存
    if((netUploadPackRecvPtr->paramBufferPtr != NULL)&&(netUploadPackRecvPtr->paramBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_NET,netUploadPackRecvPtr->paramBufferPtr);
