...
** LastEditTime  : 2023-11-03 10:20:14 +0800
************************************************************************************************/ 
...
ERROR_SUB Can2ProtAckMsgDispatch(CAN_PROT_ACK_PACK* ackPackPtr)
{
    ...
    txResult = tx_queue_send(&queueSocketCan2AckSend,(void*)(&ackPackPtr),TX_NO_WAIT);
    if(txResult == TX_SUCCESS)
    {
        //转发成功          
        return ERROR_SUB_OK;
    }
    CAN_PROT_LOG("Can2ProtAckMsgDispatch Failed,txResult: %d\r\n",txResult);
    return ERROR_SUB_BOARD_CAN2_ACK_DISPATCH_FAILED;
}
