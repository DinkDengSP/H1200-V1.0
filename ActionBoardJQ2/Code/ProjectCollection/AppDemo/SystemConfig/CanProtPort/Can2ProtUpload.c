...
** LastEditTime  : 2023-11-03 13:33:11 +0800
************************************************************************************************/ 
...
#if(CAN2_PROT_UPLOAD_SUPPORT == CONFIG_FUNC_ENABLE)
//定义上传数据转发数组 上传ID+转发消息队列指针
static const PROT_UPLOAD_PACK_FWD_UNIT can2UploadPackDispatchUnitArray[] = {
    //主控心跳上报
...
void Can2ProtUploadMsgSelfProc(CAN_PROT_UPLOAD_PACK* uploadPackPtr)
{
    ...
    Can2ProtSendAckInRecv(CAN_PROT_ACK_UPLOAD,uploadPackPtr->srcModuleNo,uploadPackPtr->srcBoardID,0,
                            uploadPackPtr->retryCount,uploadPackPtr->uploadID,PROT_ACK_OK);  
#if(CAN2_PROT_UPLOAD_SUPPORT == CONFIG_FUNC_DISABLE)
    //不支持上报ID,直接返回即可           
    return;         
#else
    ...
    //查找指令对应的信道
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(can2UploadPackDispatchUnitArray);indexUtil++)
    {
        if(uploadPackPtr->uploadID == can2UploadPackDispatchUnitArray[indexUtil].uploadID)
        {
            txQueuePtr = can2UploadPackDispatchUnitArray[indexUtil].dispatchQueuePtr;
            break;
        }
    }
    ...
    //数据关联
    sysMsgPtr->msgSrc = SYS_MSG_PACK_SRC_CAN2_UPLOAD;
