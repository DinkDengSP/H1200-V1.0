...
void SrvMsgCan1TimerUpload(void* canMsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexTimer = 0;
    for(indexTimer = 0; indexTimer < DIM_ARRAY_ELEMENT_COUNT(canMsgUploadProcessUnitArray);indexTimer++)
    {
        if(canUploadPackRecvPtr->uploadID == (uint32_t)(canMsgUploadProcessUnitArray[indexTimer].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexTimer == DIM_ARRAY_ELEMENT_COUNT(canMsgUploadProcessUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(canUploadPackRecvPtr->uploadDataBufferLength < canMsgUploadProcessUnitArray[indexTimer].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        canMsgUploadProcessUnitArray[indexTimer].exceptParamLengthMin,canUploadPackRecvPtr->uploadDataBufferLength,
    ...
    //检测函数指针
    if(canMsgUploadProcessUnitArray[indexTimer].canUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    canMsgUploadProcessUnitArray[indexTimer].canUploadProcFuncPtr(canUploadPackRecvPtr);
    //处理完成,释放内存
    if((canUploadPackRecvPtr->uploadDataBufferPtr != NULL)&&(canUploadPackRecvPtr->uploadDataBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_CAN,canUploadPackRecvPtr->uploadDataBufferPtr);
