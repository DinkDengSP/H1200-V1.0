...
//CAN1指令消息处理函数
extern void SrvMsgCan1StepMotor6Cmd(void* canMsgBodyPtr);
//CAN1上传消息处理函数
extern void SrvMsgCan1StepMotor6Upload(void* canMsgBodyPtr);
//CAN2指令消息处理函数
extern void SrvMsgCan2StepMotor6Cmd(void* canMsgBodyPtr);
//CAN2上传消息处理函数
extern void SrvMsgCan2StepMotor6Upload(void* canMsgBodyPtr);
//IPC指令消息处理函数
extern void SrvMsgIpcStepMotor6Cmd(void* ipcMsgBodyPtr);
//IPC上传消息处理函数
extern void SrvMsgIpcStepMotor6Upload(void* ipcMsgBodyPtr);
...
void SrvImplStepMotor6MsgProcess(void* taskMsgPtr)
{
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_CMD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor6Cmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN2_UPLOAD)
    {
        ...
        {
            //调用CAN消息处理函数
            SrvMsgCan2StepMotor6Upload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
