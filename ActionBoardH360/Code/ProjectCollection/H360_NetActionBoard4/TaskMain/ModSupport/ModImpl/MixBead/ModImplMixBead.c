...
//CAN1指令消息处理函数
extern void ModMsgCan1MixBeadCmd(void* can1CmdRecvPtr);
//CAN1上传消息处理函数
extern void ModMsgCan1MixBeadUpload(void* can1ReportDataPtr);
//IPC指令消息处理函数
extern void ModMsgIpcMixBeadCmd(void* ipcCmdRecvPtr);
//IPC上传消息处理函数
extern void ModMsgIpcMixBeadUpload(void* ipcReportDataPtr);
...
void ModImplMixBeadMsgProcess(void* taskMsgPtr)
{
    ...
    if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_CMD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1MixBeadCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_CAN1_UPLOAD)
    {
        ...
        {
            //调用CAN1消息处理函数
            ModMsgCan1MixBeadUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_CMD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcMixBeadCmd(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
    ...
    else if(sysMsgPtr->msgSrc == SYS_MSG_PACK_SRC_IPC_UPLOAD)
    {
        ...
        {
            //调用IPC消息处理函数
            ModMsgIpcMixBeadUpload(sysMsgPtr->msgBodyPtr);
        }
        else 
        {
            //报警,消息体为空
