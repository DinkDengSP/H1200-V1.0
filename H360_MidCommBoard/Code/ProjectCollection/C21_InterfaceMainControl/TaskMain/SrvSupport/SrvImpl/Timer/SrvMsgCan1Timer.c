...
static const USER_CAN_CMD_PROC_UNIT can1CmdProcTimerUnitArray[] = 
{
    ...
    {CAN_MASTER_CMD_PLACE_HOLDER                  ,0  ,ENABLE ,SrvMsgCan1TimerCmd_PlaceHolder  },
};


//CAN指令消息处理函数
void SrvMsgCan1TimerCmd(void* can1MsgBodyPtr)
{
    //转换指令码
    ...
    for(indexTimer = 0; indexTimer < DIM_ARRAY_ELEMENT_COUNT(can1CmdProcTimerUnitArray);indexTimer++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can1CmdProcTimerUnitArray[indexTimer].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexTimer == DIM_ARRAY_ELEMENT_COUNT(can1CmdProcTimerUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can1CmdProcTimerUnitArray[indexTimer].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can1CmdProcTimerUnitArray[indexTimer].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can1CmdProcTimerUnitArray[indexTimer].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can1CmdProcTimerUnitArray[indexTimer].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can1CmdProcTimerUnitArray[indexTimer].needResultReply == ENABLE)
    {
        //需要结果回传
...
/*************************************************************CAN上传参数处理***********************************************************/
//占位函数
static void SrvMsgCan1TimerUpload_PlaceHolder(CAN_PROT_UPLOAD_PACK* canUploadPackPtr);

//CAN上传消息处理函数列表
static const USER_CAN_UPLOAD_PROC_UNIT can1MsgUploadProcTimerUnitArray[] = 
{
...
//CAN上传消息处理函数
void SrvMsgCan1TimerUpload(void* can1MsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexTimer = 0;
    for(indexTimer = 0; indexTimer < DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcTimerUnitArray);indexTimer++)
    {
        if(canUploadPackPtr->uploadID == (uint32_t)(can1MsgUploadProcTimerUnitArray[indexTimer].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexTimer == DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcTimerUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(canUploadPackPtr->uploadDataBufferLength < can1MsgUploadProcTimerUnitArray[indexTimer].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        can1MsgUploadProcTimerUnitArray[indexTimer].exceptParamLengthMin,canUploadPackPtr->uploadDataBufferLength,
    ...
    //检测函数指针
    if(can1MsgUploadProcTimerUnitArray[indexTimer].canUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    can1MsgUploadProcTimerUnitArray[indexTimer].canUploadProcFuncPtr(canUploadPackPtr);
    //处理完成,释放内存
    if((canUploadPackPtr->uploadDataBufferPtr != NULL)&&(canUploadPackPtr->uploadDataBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_CAN,canUploadPackPtr->uploadDataBufferPtr);
