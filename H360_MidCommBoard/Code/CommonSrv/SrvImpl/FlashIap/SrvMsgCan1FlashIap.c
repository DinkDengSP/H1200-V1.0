...
void SrvMsgCan1FlashIapCmd(void* can1MsgBodyPtr)
{
    ...
    for(indexFlashIap = 0; indexFlashIap < DIM_ARRAY_ELEMENT_COUNT(can1CmdProcFlashIapUnitArray);indexFlashIap++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can1CmdProcFlashIapUnitArray[indexFlashIap].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexFlashIap == DIM_ARRAY_ELEMENT_COUNT(can1CmdProcFlashIapUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can1CmdProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can1CmdProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can1CmdProcFlashIapUnitArray[indexFlashIap].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can1CmdProcFlashIapUnitArray[indexFlashIap].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can1CmdProcFlashIapUnitArray[indexFlashIap].needResultReply == ENABLE)
    {
        //需要结果回传
...
void SrvMsgCan1FlashIapUpload(void* can1MsgBodyPtr)
{
    ...
    //搜索匹配处理器
    uint16_t indexFlashIap = 0;
    for(indexFlashIap = 0; indexFlashIap < DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcFlashIapUnitArray);indexFlashIap++)
    {
        if(canUploadPackPtr->uploadID == (uint32_t)(can1MsgUploadProcFlashIapUnitArray[indexFlashIap].uploadId))
        {
            break;
        }
    }
    //判定序号
    if(indexFlashIap == DIM_ARRAY_ELEMENT_COUNT(can1MsgUploadProcFlashIapUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(canUploadPackPtr->uploadDataBufferLength < can1MsgUploadProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,UploadID: 0X%08X\r\n",__func__,__LINE__,
                        can1MsgUploadProcFlashIapUnitArray[indexFlashIap].exceptParamLengthMin,canUploadPackPtr->uploadDataBufferLength,
    ...
    //检测函数指针
    if(can1MsgUploadProcFlashIapUnitArray[indexFlashIap].canUploadProcFuncPtr == NULL)
    {
    ...
    //调用指令处理
    can1MsgUploadProcFlashIapUnitArray[indexFlashIap].canUploadProcFuncPtr(canUploadPackPtr);
    //处理完成,释放内存
    if((canUploadPackPtr->uploadDataBufferPtr != NULL)&&(canUploadPackPtr->uploadDataBufferLength != 0))
    {
        UserMemFree(MEM_REGION_SYS_MSG_CAN,canUploadPackPtr->uploadDataBufferPtr);
