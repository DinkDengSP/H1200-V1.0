...
** LastEditTime  : 2023-10-31 17:13:10 +0800
************************************************************************************************/ 
...
//单个参数读取
static void SrvMsgCan2ParamReadWriteCmd_ReadSingle(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//批量参数读取
static void SrvMsgCan2ParamReadWriteCmd_ReadAny(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//单个参数写入
static void SrvMsgCan2ParamReadWriteCmd_WriteSingle(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
//批量参数写入
static void SrvMsgCan2ParamReadWriteCmd_WriteAny(CAN_PROT_CMD_PACK* cmdPackPtr,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen);
...
static const USER_CAN_CMD_PROC_UNIT can2CmdProcParamUnitArray[] = 
{
    ...
    {CAN_MASTER_CMD_PARAM_RW_READ_SINGLE     ,2  ,ENABLE ,SrvMsgCan2ParamReadWriteCmd_ReadSingle  },
    {CAN_MASTER_CMD_PARAM_RW_READ_ANY        ,2  ,ENABLE ,SrvMsgCan2ParamReadWriteCmd_ReadAny     },
    {CAN_MASTER_CMD_PARAM_RW_WRITE_SINGLE    ,6  ,ENABLE ,SrvMsgCan2ParamReadWriteCmd_WriteSingle },
    {CAN_MASTER_CMD_PARAM_RW_WRITE_ANY       ,6  ,ENABLE ,SrvMsgCan2ParamReadWriteCmd_WriteAny    },
};
...
void SrvMsgCan2ParamCmd(void* can2MsgBodyPtr)
{
    ...
    for(indexUtil = 0; indexUtil < DIM_ARRAY_ELEMENT_COUNT(can2CmdProcParamUnitArray);indexUtil++)
    {
        if(cmdPackPtr->cmdValue == (uint32_t)(can2CmdProcParamUnitArray[indexUtil].commandCode))
        {
            break;
        }
    }
    //判定序号
    if(indexUtil == DIM_ARRAY_ELEMENT_COUNT(can2CmdProcParamUnitArray))
    {
        //不支持的指令
    ...
    //判定长度
    if(cmdPackPtr->paramBufferLength < can2CmdProcParamUnitArray[indexUtil].exceptParamLengthMin)
    {
        //小于最小指令预期长度
        SystemPrintf("E:> %s,%d,Recv ExceptMinParamLength:%d ,Real: %d,CommandCode: 0X%08X\r\n",__func__,__LINE__,
                        can2CmdProcParamUnitArray[indexUtil].exceptParamLengthMin,cmdPackPtr->paramBufferLength,
                        cmdPackPtr->cmdValue);
        //释放内存
        if((cmdPackPtr->paramBufferPtr != NULL)&&(cmdPackPtr->paramBufferLength != 0))
        {
    ...
    //检测函数指针
    if(can2CmdProcParamUnitArray[indexUtil].canCmdProcFuncPtr == NULL)
    {
    ...
    //调用处理函数
    can2CmdProcParamUnitArray[indexUtil].canCmdProcFuncPtr(cmdPackPtr,&resultPack,&resultDataBufferPtr,&resultDataLength);
    //结果处理,是否需要结果回传
    if(can2CmdProcParamUnitArray[indexUtil].needResultReply == ENABLE)
    {
        //需要结果回传
