...
void SrvImplBaseParamReadSingle(uint8_t mainIndex,uint8_t subIndex,SYS_RESULT_PACK* resultPackPtr,uint8_t** dataBufPtrPtr,uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = UserMemMallocWhileSuccess(MEM_REGION_SRV_TASK,*dataBufLen);
    ...
    //调用相应的API读取
    errorSub = SystemParamReadSingle((INDEX_MAIN_SYS_PARAM)mainIndex,subIndex,&paramReadInt32);
                                    ...
                                    uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = UserMemMallocWhileSuccess(MEM_REGION_SRV_TASK,*dataBufLen);
    ...
    {
        errorSub = SystemParamReadSingle((INDEX_MAIN_SYS_PARAM)(readParaPairDataArrayPtr[indexParam*2]),
                                        ...
                                        uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    ERROR_SUB errorSub = ERROR_SUB_OK;
    //数据写入
    errorSub = SystemParamWriteSingle((INDEX_MAIN_SYS_PARAM)mainIndex,subIndex,writeParam);
    if(errorSub != ERROR_SUB_OK)
    {
        //报错
                                    ...
                                    uint16_t* dataBufLen)
{
    ...
    *dataBufPtrPtr = NULL;
    ...
    for(indexParam = 0; indexParam < writeParamPair; indexParam++)
    {
        ...
        //执行写入
        errorSub = SystemParamWriteSingle((INDEX_MAIN_SYS_PARAM)(writeParamPairArrayPtr[indexParam*6]),
